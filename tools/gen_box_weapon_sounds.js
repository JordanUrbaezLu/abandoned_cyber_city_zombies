...
// sid = sound folder / alias prefix; shot[] = fire wav filenames (under fire\);
...
for ( const g of GUNS ) {
    ...
    const foleyDir = path.join( SA, g.sid, "foley" );
    if ( fs.existsSync( foleyDir ) ) {
        for ( const f of fs.readdirSync( foleyDir ).filter( ( f ) => f.toLowerCase().endsWith( ".wav" ) ) ) {
            ...
            out.push( mkRow( tmplFoley, name, `skye_ports\\${g.sid}\\foley\\${f}` ) );
            nFoley++;
        }
    } else {
