...
const MAP = path.join(__dirname, '..', '..', 'map_source', 'zm', 'zm_abandoned_cyber_city.map');   // (fixed 2026-07-12: the oneshots/ move left this one '..' short)
...
// REAL SKINS: WALLS + FLOORS/CEILINGS/STAIRS all = t7_metal_worn_iron_dark (user 2026-07-13:
// "the floor and ceilings of the trench levels should use the same material as the walls of the
// bus station trench" - the L1 corp/bus-station trench walls are t7_metal_worn_iron_dark, so the
// whole descent now reads as one continuous dark-metal shaft matching L1). Supersedes the
// 2026-07-12 asphalt floor. STOCK, ships free, no .zone line. Face-token swap = BSP-baked -> FULL build.
const F = 't7_metal_worn_iron_dark', W = 't7_metal_worn_iron_dark';
...
function emitWellWalls(k, w) {
  // rail = cz + 224 = FLOOR-TO-CEILING (user 2026-07-13: was cz+128, which left a ~96u gap under the
  // 224u ceiling so a double-jump could hop the railing INTO the well and down the stairs, bypassing
  // the soul door = descend for free). 224 = the layer headroom -> the railing meets the ceiling slab,
  // fully jump-proof. MUST match gen_abyss_doors.js DOOR_H (224). Same proven box() winding, just taller.
  const fz = floorZ(k + 1), cz = floorZ(k), rail = cz + 224;   // railing top = the ceiling above this floor
...
// NO lights below the pit - PERMANENT (user 2026-06-22 "remove the lights in the trenches
// completely"; RE-CONFIRMED user 2026-07-12 "you added lights which I didn't even want"
// after a brief Infected-Descent re-light was shipped without asking - that was wrong,
// reverted same day; the 10 new baked lights were also a suspect in the +0x2bc4ee9
// post-spawn CTD). The abyss reads by SELF-EMISSIVE props only. Do not flip this again
