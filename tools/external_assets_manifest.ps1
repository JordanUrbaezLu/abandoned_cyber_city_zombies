...
$ExternalAssetPacks = @(
    ...
    @{
        ...
        Provides = 'Tac-19, Five-seven, ASM1, AK-47, AE4, Ripper (+ FAL), s4_klauser (VG Luger, C-tier box pistol, user 2026-07-05). PATCH AFTER (RE)INSTALL of Skye_VG_Klauser: the s4_klauser_up (PaP) block ships a baked Nydar reflex optic whose vm_/wm_s4_reflex_nydar models are absent -> the PaP weapon FAILS to load. Blank the 6 optic fields on the _up block (attachViewModel1/2, attachWorldModel1, attachViewModelTag1/2, attachWorldModelTag1 -> "") to match the base = iron sights, then gdtdb /update. Backup skye_s4_klauser.gdt.acc-optic-orig. (Same install-side-GDT-patch caveat as the AK-74u altWeapon fix - not repo-tracked.) The 2 leftover mtl/i_optic_nydar_00 errors are cosmetic/waived (weapon loads). ALSO s1_mdl (the AW MDL) is now used - RESKINNED into the EPG-1 plasma-lobber: RE-RUN tools/oneshots/reskin_mdl_epg.js after any Skye reinstall (retunes projectileSpeed arc + normalizes the _up raw to base + repaints explosion/trail/muzzle FX to stock EMP/energy; backup skye_s1_mdl.gdt.acc-epg-orig), then gdtdb /update. (launcher_multi/L4 Siege needs NO Skye asset - it is stock-cooked in zm_levelcommon.)'
        Required = $true
    ...
    @{
        Name     = 'MidgetBlaster T7 Assets V2.8 (drone slice - Maxis quadrotor helper-drone pieces)'
        Author   = 'MidgetBlaster (T7 rips; tools: Spiki, Scobalula, Serious)'
        Provides = 'HELPER-DRONE body xmodels (carved 2026-07-22): veh_t7_dlc_zm_quadrotor_piece_body (Origins Maxis Drone chassis, 35.8x8.7x11.7u - THE hovering helper-drone body) + _piece_brain (brain jar 10.4x10.4x6.9u) + _piece_fan (rotor assembly 18.3x18.3x14.0u). All three are the STATIC buildable-piece variants (tag_origin-free LOD0 bins); the assembled veh_t7_dlc_zm_quadrotor is RIGGED at ALL 8 LODs (45 tags) and was NOT carved - never swap it in (converter fail, same as the veh_t7_drone_uav/raps/attack family, all rigged all LODs, screened 2026-07-22). GDT auto-generated by tools/gen_t7_carve_gdt.js; shared micro/scuffed + body01_g maps resolve from fastfiles (canonical stock names, no image entries).'
        Required = $true   # helper-drone system: zone xmodel, lines + GSC precache reference these once wired
        Marker   = 'source_data\acc_t7_drone.gdt'
        Link     = 'User rar "T7 Assets V2.8.rar" (Downloads) - keep compressed as the prop library; extract per-prop slices on demand (staging: Downloads\_t7stage)'
        Paths    = @(
            'source_data\acc_t7_drone.gdt',
            'model_export\_midgetblaster\props\p7_zm_tomb'
        )
    },
    @{
    ...
    @{
        Name     = 'D13 Sector disc launcher (Koentje Disk Gun)'
        Author   = 'KOENTJE (port; credits HarryBo21/D-2-K/Scobalula/DTZxPorter) / Treyarch (orig T7 loot special)'
        Provides = 'special_discgun + special_discgun_up projectileweapons (view/world/anim xmodels + projectile model, ~50 view xanims, PBR images, GDT source_data\koentje\koentje_disk_gun.gdt). INSTALL-SIDE FIXES baked (2026-07-24): rpg.accu accuracy refs -> default.accu (aivsplayer\rpg.accu is NOT stock = silent weapon drop; .acc-orig backup beside the GDT) + the pack sound CSV was surgically trimmed/re-donored into repo sound/aliases/discgun_sounds.csv (16/18 wpn\energy\discgun wavs never shipped - whole-bank risk; fire=apex b3wing, lfe=spike_launcher, act=shipped disc foley, water/under+bounce/loop rows deleted). Re-apply BOTH fixes on a reinstall.'
        Required = $true   # zone references weapon,special_discgun / _up -> a fresh clone link-errors without it
        Marker   = 'model_export\_koentje\disk_gun\t7_disk_gun_view.xmodel_bin'
        Link     = 'Koentje_Disk_Gun_v1.0.1.zip (44 MB, user Downloads 2026-07-24) - MEGA qtxXgCwB#LGcLsBUU... via the icegrenade.co.uk ZGC directory (link live 2026-07-24, fetched with megajs).'
        Paths    = @(
            'model_export\_koentje',
            'xanim_export\_koentje',
            'source_data\koentje\koentje_disk_gun.gdt',
            'sound_assets\fly\weapon\reload\disc',
            'share\raw\sound\aliases\discgun_sounds.csv'
        )
    },
    @{
        Name     = 'HB21 Hero Weapons v2.0.0 (Dragon Gauntlet + Skull of Nan Sapwe subset)'
        Author   = 'HarryBo21 + pack credits (D-2-K, Yen466, Lilrifa, Scobalula, DTZxPorter, et al.) / Treyarch (orig DLC hero weapons)'
        Provides = 'ZM hero-weapon assets: keeper-head/skull + dragon-gauntlet world models + GDTs (wpn_t7_zmb_dlc2_keeper_head, wpn_t7_zmb_dlc3_gauntlet_dragon, c_zom_dlc3_dragon whelp vehicle), share/raw fx (skull beam/torch suite, gauntlet flame/glove glows), accuracy files, collmaps, t7_hero_weapon_sounds.csv + sound_assets. Weapon DEFS link from the tools DLC asset db (weapon,skull_gun_zm etc.). Scripts are VENDORED in git (scripts/zm/_hb21_zm_hero_weapon.* + _zm_weap_dragon_gauntlet.* + _zm_weap_keeper_skull.* - the skull carries the [acc] toplayer->allplayers CF surgery; NEVER install the pack''s "usermaps - OPEN ME" copies over them, and NEVER add its gravityspikes/glaive modules (scriptmover/toplayer pools FULL)).'
        Required = $true   # zone references the gauntlet/skull fx + elbow xmodels + t7_hero_weapon_sounds szc source
        Marker   = 'source_data\wpn_t7_zmb_dlc2_keeper_head.gdt'
        Link     = 'hb21_specialist_weapons_v2.0.0.rar (283 MB, user Downloads 2026-07-24) - MEGA WD4y1CpZ#qpa6wyqf... (link live 2026-07-24, fetched with megajs). Install = everything EXCEPT "usermaps - OPEN ME" + zm_EXAMPLE.szc.'
        Paths    = @(
            'source_data\wpn_t7_zmb_dlc2_keeper_head.gdt',
            'source_data\wpn_t7_zmb_dlc3_gauntlet_dragon.gdt',
            'source_data\c_zom_dlc3_dragon.gdt',
            'share\raw\sound\aliases\t7_hero_weapon_sounds.csv',
            'share\raw\fx\dlc2\zmb_weapon',
            'share\raw\fx\dlc3\stalingrad',
            'sound_assets\wpn\hero'
        )
    },
    @{
        Name     = "Logical's Cyberpunk Riot Shield (reskin mesh)"
        Author   = "Logical (shield mesh - original custom; bench = BO4 rip) / Treyarch (BO4 crafting table)"
        Provides = 'logical_m_shield_full/part_01/part_02 xmodels + PBR images (GDT copied to source_data\logical_models_crafting.gdt for gdtdb) + p8_zm_crafting_table BO4 bench models/GDT + crafting prefabs. WE USE ONLY THE MESH: source_data/acc_riotshield_reskin.gdt (repo, custom-authored) shadows the 6 stock wpn_t7_zmb_zod_rocket_shield_* model names with the cyber shield - the Rocket Shield boss item keeps native zod_riotshield mechanics. The pack''s crafting system (log_craft scripts/prefabs/sounds) is NOT wired.'
        Required = $true   # zone xmodel, lines force-pack the 6 shadow models; their GDT filename field points at this mesh
        Marker   = 'model_export\_logical_models\logical_models\crafting_assets\logical_m_yaw_shield_full.xmodel_bin'
        Link     = 'Logicals_Riot_Shield.rar (88 MB, user Downloads 2026-07-24) - Google Drive 1x7WZ-KXU6W7MZN1ikST4cA1TzyEW86Kl (live, curl-downloaded 2026-07-24).'
        Paths    = @(
            'model_export\_logical_models',
            'model_export\p8_zm_crafting_table',
            'source_data\p8_zm_crafting_table.gdt'
        )
        # NOTE: logical_models_crafting.gdt lives INSIDE model_export\_logical_models (gdtdb
        # scans model_export GDTs - Avogadro precedent); a source_data copy = duplicate-asset error.
    },
    @{
        Name     = 'Coolyer zm_slots icons (CYBER SLOTS machine)'
        Author   = 'Coolyer (script + packaging; adapted into scripts/zm/zm_abandoned_cyber_city/_acc_slots.gsc) / Flaticon artists (icons: IconsNova, Smashicons, Laura Reen, Freepik, Ian Anandara - attribution per Flaticon licence)'
        Provides = '11 slot-reel icon materials (cherry/lemon/bar/seven/bell/diamond/clover/coin/banana/skull/death) as _custom\_coolyer\gambling images + ui_gambling_icons.gdt (picked up via the converter_gdt_dirs _custom bin patch). The GSC is NOT installed from the pack - our adapted _acc_slots module lives in git.'
        Required = $true   # zone material, lines for the 11 icons -> a fresh clone link-errors without it
        Marker   = '_custom\_coolyer\gambling\ui_gambling_icons.gdt'
        Link     = 'https://github.com/coolyer/zm_slots/releases/download/1.2/coolyer.zm_slots.zip (public GitHub release - re-downloadable, also in the teammate zip).'
        Paths    = @(
            '_custom\_coolyer\gambling'
        )
    }
)
