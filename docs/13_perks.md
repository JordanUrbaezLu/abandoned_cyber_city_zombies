...
- **Mega — PhD Slider**: adds the **slide** nova + a bigger down nova at radius **500u**, **~0.8× the base-nova damage** (`ACC_PHD_MEGA_DAMAGE_MULT 0.8`, nerfed −60% from a flat 2× on 2026-06-24), and an **8s** slide cooldown (base would be 10s). **Also 1.5× SLIDE speed** (`acc_mega_flopper_speed` → `acc_utility::recompute_move_speed`, slide-gated, re-applied on respawn) and **+15% explosive damage** (grenades / projectiles / `MOD_EXPLOSIVE` via `_acc_damage::on_ai_damage` `is_explosive_mod` + `has_active_mega_perk` — GSC scalar, no twin). **Slide speed stacks multiplicatively** with the Rocket Shield slide bonus: sliding with both = `1.5 × 1.35 ≈ ×2.0` move scale, plus the shield's forward lunge.
...
| | Mega PhD Slider (**slide-to-explode** + bigger/stronger slide + down explosion) | OK | **Slide-to-explode is MEGA-ONLY (user 2026-06-26)** — base no longer detonates on a slide (`phd_slide_watcher` gates on the Mega flag; dvar `acc_phd_base_slide_nova`). Mega tier read live from the Mega flag: radius 300→500u, **~0.8× base-nova damage** (NERFED −60% from 2× on 2026-06-24, `ACC_PHD_MEGA_DAMAGE_MULT 0.8`), **10s→8s** slide cooldown, +15% explosive, ×1.5 slide speed |
