...
| | ~~Mega Spiderman boss-special immunity~~ | MOVED | Boss-special immunity moved OFF Mega Widow's to **Mega Electric Cherry "Power Surge"** (2026-06-25) — `_acc_boss::protect_immune_players_during_debuff` (EMP/power-off: still full immunity) + `_acc_elites::acc_phantom_chain_zap`/`acc_protector_zap` gate on `specialty_combat_efficiency`. **Boss STUN softened, not immune (user 2026-07-03):** Mega Electric Cherry now takes the Phantom/Rogue-Protector slow to **-10%** (`acc_boss_slow_mega_mult 0.90`) instead of 0%, vs the normal **-25%** (`acc_phantom_slow_mult`/`acc_protector_slow_mult 0.75`) |
