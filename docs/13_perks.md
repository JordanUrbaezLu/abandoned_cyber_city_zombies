...
| 4 | **Double Tap 2.0** | **3,000** | Fires **2 bullets per shot for 1 round of ammo** (≈**2× damage**; double pellets on shotguns) **plus +33%** rate of fire. Excludes Wonder Weapons / Ballistic Knife / explosives. | **Gun Slinger** | **+45%** fire rate; **weapon-swap time −50%** (≈2× faster swaps). |
...
| 8 | **Widow's Wine** | 4,000 | Web grenades (web un-killed zombies ~20 s); self-defense webbing on hit; webbing melee; stock per-round grenade restock. | **Spiderman** | **one-hit melee on regular zombies** (not bosses/elites) + **low-stance speed** (crouch ×2.6 / prone ×10 / downed ×15). *(Boss-special immunity MOVED to Mega Electric Cherry "Power Surge", user 2026-06-25. The custom 6-grenade pool + 4/round restock + WEB GRENADES HUD were REMOVED 2026-06-24 — web grenades use stock behavior now.)* |
| 9 | **PhD Flopper** | 2,500 | **Immune to fall damage and your own explosive / grenade / projectile splash**; **explode when you go down** (PhD-flavoured last-stand; orange `def_explosion` nova). *(Slide-to-explode is **Mega-only** since 2026-06-26.)* | **PhD Slider** | Adds **slide-to-explode** (Mega-exclusive) + a nova — radius **300→250u** (HALVED, user 2026-06-27), **≤10 zombies hit/slide** (`acc_phd_max_hits`), **10s→8s** cooldown; nova **damage FROZEN at round-16 zombie HP** (one slide one-shots trash ≤r16, then 2/3 slides past it; `acc_phd_freeze_round`); **1.5× slide speed**; **+15% explosive damage**. |
| 10 | **Electric Cherry** | 3,000 | **Reloading discharges an electric nova** that zaps nearby zombies — the **emptier the mag, the bigger the blast** (an empty-mag reload one-shots trash at any round); on a short cooldown. *(Custom from-scratch perk on `specialty_combat_efficiency`, real `p6_zm_vending_electric_cherry` model. No explode-on-down — PhD owns that.)* | **Power Surge** | **+50% damage, more targets (12), faster recharge (5 s)** — the Mega's edge is damage/targets/cooldown, **not** blast size (the empty-mag radius is actually a touch tighter than base, by design: 200 vs 220). AND **immune to boss "special moves"** — live: the **Phantom chain-zap slow** (also the Subroutine Core power/perk-disable, for when that full boss is active; immunity moved here from Mega Widow's Wine, user 2026-06-25). |
...
**Mega: Ultimate Tank.** Max health to **300 HP** → survive **6** hits, **down on the 7th**. *(The boss-special immunity that used to be here was MOVED to Mega Widow's Wine "Spiderman" — user 2026-06-24.)*
...
**Base (Double Tap 2.0).** **Fires 2 bullets per shot for the cost of 1 round of ammo** (double pellets on shotguns) **plus +33% rate of fire** (stock). Untouched, the extra bullet would stack to ~2.66× DPS (2× dmg × 1.33 RoF), so we **temper the per-hit DAMAGE** (user 2026-06-25): `acc_doubletap_dmg_mult = 0.6` → base DT lands ≈ **1.6× DPS** (2 bullets × 0.6 × 1.33). The fire rate is left intact. Does **not** apply to Wonder Weapons, the Ballistic Knife, or explosive weapons.

**Mega: Gun Slinger.** **+45% fire rate** (user 2026-06-25, was +40%; on top of the 2.0 base); **weapon-swap time reduced 50%** (≈2× faster weapon swaps — moved here from Speed Cola).

**Mechanics**

- **Damage temper (`acc_doubletap_dmg_mult`, 0.6):** a per-hit REDUCTION in `_acc_damage.gsc` on Double-Tap holders (base AND Mega), so the engine extra bullet nets ≈**1.6× DPS** instead of ~2.66×. **100%-safe allow gate** (`weapon_gets_dt_bullet`): applied to every gun **except the Thundergun + Mahem**, and only on bullet hits (melee / grenades / equipment auto-excluded) — so it can never nerf a weapon that lacks the extra bullet. (VERIFY in-game that MORS / Paladin fire 2 rounds with DT — two crosshair damage numbers; if not, add them to the deny-list.)
- **Fire rate:** base **+33%** → Gun Slinger **+45%** (`fastfire` twin, `fireTime ×0.69`).
...
- **Headshot temper** is applied **multiplicatively** to a head hit: `locHead × 0.5` on trash (`ACC_HEADSHOT_MULT`) / `locHead × 0.6` on boss (`ACC_BOSS_HEADSHOT_MULT`). Most box guns are `locHead 5.0` → **×2.5 trash / ×3 boss** with no perk (user 2026-06-25). **Deadshot's bonus is SEPARATE and additive** — it adds **+1.3** (base) / **+1.5** (American Sniper) into the crit-damage bonus pool on top (PaP/Cyberware also add into that pool).
...
**Mega: PhD Slider.** Adds the **slide-to-explode** (Mega-exclusive — base PhD doesn't detonate on a slide) + a **bigger/stronger nova** on a **shorter slide cooldown** (base **10s** → Mega **8s**). Radius **300→250u** (Mega HALVED, user 2026-06-27), **capped at 10 zombies hit per slide** (`acc_phd_max_hits`); Mega nova damage **FROZEN at a round-18 zombie's health** (`ACC_PHD_FREEZE_ROUND 18`, dealt raw via the `acc_phd_nova_hit` bypass — one slide one-shots trash ≤r18, then 2/3 slides past it; replaces the old ~0.8× live-scaled). **Plus 1.5× SLIDE speed** (slide-gated, like the Rocket Shield — stacks multiplicatively with the shield's 1.35× slide bonus → ~**×2.0** with both) and **+15% explosive damage** (a GSC scalar in `_acc_damage`, **no weapon twin**). Working Mega — the Empty Mega Bottle sets the flag and the bigger nova fires immediately.
...
- **Mega — PhD Slider**: adds the **slide** nova + a down nova at radius **250u** (user 2026-06-27, HALVED from 500), **capped at 10 zombies hit per slide** (`acc_phd_max_hits`), **damage FROZEN at a round-16 normal-zombie's health** (`ACC_PHD_FREEZE_ROUND 16`, co-op-scaled, dealt raw via the `acc_phd_nova_hit` bypass — one slide one-shots trash through r16, then 2 slides ~r17–23, then 3; user 2026-06-27, replaces the old live-scaled ×0.8), and an **8s** slide cooldown (base would be 10s). **Also 1.5× SLIDE speed** (`acc_mega_flopper_speed` → `acc_utility::recompute_move_speed`, slide-gated, re-applied on respawn) and **+15% explosive damage** (grenades / projectiles / `MOD_EXPLOSIVE` via `_acc_damage::on_ai_damage` `is_explosive_mod` + `has_active_mega_perk` — GSC scalar, no twin). **Slide speed stacks multiplicatively** with the Rocket Shield slide bonus: sliding with both = `1.5 × 1.35 ≈ ×2.0` move scale, plus the shield's forward lunge.
...
- **Mega too strong**: reduce specific Mega effects (e.g. American Sniper +1.5 → +1.4 headshot bonus, Gun Slinger +45% → +40% fire rate).
...
**All 9 perks are consolidated to the Laboratory**, each in its **own door-gated alcove** on the Lab north wall (nowhere else on the map). **IMPLEMENTED 2026-06-16** (`tools/add_perk_alcoves.js` geometry + `_acc_perk_doors.gsc`): every round, a **random 4 of the 9 alcove doors open** (user 2026-06-23: 3 → 4, more perk access) (`acc_perk_door_<specialty>` `script_brushmodel` gates); the other 5 are walled off and **unbuyable that round**. The roll re-shuffles each round (`acc_round_start`). A closed door blocks *access to the machine* only — a perk you already own keeps working. **Dev mode: ALL alcoves open** (user 2026-06-26) — `acc_resolve_dev_flags()` `SetDvar`s `acc_perk_doors_all_open 1` off `level.acc_dev`, which `dev_all_open()` reads, so the rotation is bypassed and every perk is buyable while testing. (This reverses the earlier 2026-06-18 "walls close in dev too" choice; before it, dev was wrongly documented as following `acc_open_map` but the code only ever read `acc_perk_doors_all_open`, which dev never set.) Normal play (`acc_dev 0`) leaves the dvar at the ship default 0 and the per-round rotation runs; force all open by hand with `set acc_perk_doors_all_open 1`.
...
**Tuning levers:** `ACC_PERK_DOORS_OPEN_PER_ROUND` in `_acc_perk_doors.gsc` (default **4**-of-9, user 2026-06-23, was 3) — raise further if a starved feel emerges. The doors currently re-roll on `acc_round_start` (round start), NOT gated to the decontamination-complete tick the old plan used; revisit if the timing should align with decon.
...
- **Fire rate**: +33% (Double Tap 2.0 base) / +45% (Gun Slinger); Gun Slinger also cuts weapon-swap time 50%. Base also fires an extra bullet per shot, each tempered to 0.6× → ~1.6× DPS.
...
- **Jugger-Nog** — ✅ base 250 HP (`_acc_perks.gsc` `ACC_JUGG_HEALTH_ADD=150`); ✅ Ultimate Tank 300 HP (`_acc_mega_bottles.gsc` `n_player_health_boost=50`). *(Boss-special immunity now lives on **Mega Electric Cherry "Power Surge"** — moved Jug→Widow's→Electric Cherry, settled 2026-06-25; `_acc_boss.gsc::protect_immune_players_during_debuff` gates on `specialty_combat_efficiency`.)* 🧪 confirm 6/7 hit counts.
...
- **Electric Cherry** — ✅ **live.** The **real 10th perk**, a from-scratch ability on the unused `specialty_combat_efficiency` (NOT the cherry pipeline PhD hijacks): `_acc_perk_electric_cherry.gsc`, cost **3,000**, own Lab machine (`vending_acc_electric_cherry`, model `p7_lab_bio_machinery_01`) + own perk door + red/teal perk-bar icon (`i_acc_perk_cherry_{base,mega}`). ✅ reloading fires an electric nova electrocuting nearby zombies, radius/damage scaling with clip emptiness (empty mag = big blast — fixes the stock 1/10 stub); **8** targets / **6s** cooldown; survivors get the real tesla stun + shock FX. ✅ **Power Surge** Mega (read live off the Mega flag): **+50% damage, 12 targets, 5s cooldown** (radius NOT raised — `EC_RADIUS_MAX_MEGA` 200 ≤ base 220, by design — its edge is damage/targets/cooldown), **AND boss-special immunity** (Phantom chain-zap slow + Subroutine-Core power/perk-disable; `_acc_boss.gsc` + `_acc_elites.gsc` gate on the **persistent** `specialty_combat_efficiency` Mega flag). Card "Electric Cherry" / "Power Surge". LUI card index **10** (PaP shifted to 11).
...
| | base bullet damage ×0.6 temper | OK | `_acc_damage.gsc:392-398` (`ACC_DOUBLETAP_DMG_MULT 0.6`, user 2026-06-25): the stock extra bullet can't be stripped in a usermap, so each DT bullet is cut to 0.6× → net **~1.6× DPS** (2 × 0.6 × +33% RoF), not a flat 2×. Allow-list `weapon_gets_dt_bullet` (bullet guns only); base AND Mega. *(The old +3% damage layer was removed 2026-06-14.)* |
| | cost 3,000 | OK | `set_perk_costs` (`specialty_doubletap2 = 3000`; 5,000 → 3,000 user 2026-06-25) |
| | Mega Gun Slinger +45% fire rate | DONE | **No runtime lever** (grep-proven: no fire-rate setter/dvar anywhere; stock DT2's +33% is hardcoded to the specialty, `_zm_perk_doubletap2.gsc` has zero rate code). Done via the **`fireTime` weapon-variant swap** — the `fastfire` twin (`fireTime ×0.69`, +40%→+45% user 2026-06-25, [apply_recoil_overhaul.js](../tools/apply_recoil_overhaul.js) `fire 0.69`) is `GiveWeapon`'d while Gun Slinger is active (`axis_fire`) ([doc 30](30_perk_gdt_radiant_spec.md)) |
| | ~~Mega Gun Slinger +6% damage~~ | REMOVED | Removed with the DT damage layer (2026-06-14); Gun Slinger = +45% fire rate + −50% weapon-swap only |
...
| | ~~Mega Spiderman 6 web grenades~~ | REMOVED | The custom 6-grenade virtual pool + 4/round restock + WEB GRENADES HUD were **removed 2026-06-24** (user). Mega Widow's web grenades use **stock** behavior now (the GDT carry cap was never raisable, docs/30 abandoned). |
| | ~~Mega Spiderman boss-special immunity~~ | MOVED | Boss-special immunity moved OFF Mega Widow's to **Mega Electric Cherry "Power Surge"** (2026-06-25) — `_acc_boss::protect_immune_players_during_debuff` + `_acc_elites::acc_phantom_chain_zap` now gate on `specialty_combat_efficiency` |
...
- **Mega Bottle system — OK; effect-application now near-complete.** drop / inventory
  ...
  **8 fire** (Savior, The Flash sprint, The Armory fill, Spiderman, Ultimate Tank,
  ...
  radius 300→250u / frozen round-16-HP damage / ≤10 hits per slide, read live from the Mega flag). **Sleight of Hand Expert**
...
- **APE GUI (weapon GDT, interactive — can't be done headlessly):** Widow 6-web-grenade
  ...
  **Gun Slinger +45% fire rate** (both are `_norecoil`/`_fastfire` **weapon-variant
