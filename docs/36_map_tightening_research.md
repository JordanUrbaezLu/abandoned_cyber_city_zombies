...
| **Spawn & Lab never sealed; both Lab approaches intact** | `docs/03:17,54,92-93` | One Lab approach welds per run — keep both physically built. |
...
- **Verify:** Spawn→Lab path intact through Corp; both trains still loopable; power switch reachable; decon never seals Corp.
- **Build type:** full.
...
- [ ] **Zone reads** — dev zone HUD reads `UNDERCITY MARKET` in the shrunk room (`_acc_dev.gsc:391-439`).
