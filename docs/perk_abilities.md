...
**Mega — PhD Slider:**
- **Slide-to-explode (Mega-exclusive) + bigger blasts** — **only PhD Slider detonates on a slide** (base PhD doesn't; user 2026-06-26). The slide/down nova radius is **300 → 500u**, Mega slide cooldown **8s** vs base **10s** (user 2026-06-22, was 5s/8s). The Mega nova **damage** used to be a flat **2× the base nova**, but the base nova is already round-scaled to one-shot trash, so the ×2 (compounded by the global ×2.5 buff + the Mega Flopper +15% explosive) read **~64k — absurd**. **Nerfed −60%** (user 2026-06-24): the Mega nova is now **~0.8× the base nova** (`acc_phd_mega_dmg_mult`, default 0.8) — still one-shots trash via the global buff; the Mega's real edge is now its radius / cooldown / move-speed, not raw nova damage. Live from the Mega flag.
