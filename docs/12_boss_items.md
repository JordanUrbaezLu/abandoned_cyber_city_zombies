...
- **Regular zombies** also have a tiny drop chance (user 2026-06-27; rates halved 2026-06-29): every non-boss zombie death independently rolls **0.2%** to drop a random pool item (free-for-all world pickup) **and** **0.2%** to grant **one Empty Mega Bottle to the killer only** (direct grant, not shared). **The Lucky Clover raises BOTH to 0.3%** (×1.5) for its carrier — so 0.2% normally, 0.3% with the Clover. Bosses/mini-bosses are excluded (they keep their guaranteed drops). Live dvars: `acc_zombie_item_drop_chance` / `acc_zombie_bottle_drop_chance` (default `0.002`). Code: `_acc_boss_items::on_zombie_death_drop`.
...
| 2 | **Loot Stash** | gold brick (`zombietron_gold_brick`) | **+10 pts/kill, flat** — NO headshot tier, and **Double Points does NOT boost it** (DP still ×2's the base kill points, just not this bonus; user 2026-06-29 nerf, was 10/15 +5/+10 DP). A **Nuke still pays the holder 500** (1000 w/ Double Points). | Passive, KILLER only. Logic in `_acc_points` (`award_killer_with_ledger` + `ledger_nuke_watch`). |
| 3 | **Repair Kit** | carpenter icon | **+10 HP/sec** passive health regen | Passive (caps at max health; pauses while downed). |
| 4 | **Rocket Shield** | rocket shield | **Mobility** — **+50% speed while sliding** (×1.5), a forward lunge on slide-start, **2× jump height** (×1.42 velocity ≈ 2× apex) | Passive — just slide and jump. (user 2026-06-29 buff: slide 1.35×→1.5×; dvars `acc_rocket_slide_mult` / `acc_rocket_jump_mult` / `acc_rocket_slide_kick`) |
| 5 | **Phase Serum** | perk-bottle vial | **Glitch-suppression aura** (user 2026-06-29 nerf, was a cloak) — any **Glitch Stalker within ~350u is slowed to 1/5 speed AND loses its blink** (its glitch ability). It can still SEE + chase you — just nullified, not blinded. Regular horde unaffected. | Passive. Dvars `acc_phase_serum_radius` (350) / `acc_phase_serum_slow` (0.2). Logic in `_acc_boss_glitch::acc_serum_suppressed`. |
...
| 7 | **Lucky Clover** | X2 orb (`p7_zm_power_up_double_points` — placeholder; the X2 reads as "double luck") | **Drop luck** — while implanted, YOUR kills raise the zombie random-item + Mega-Bottle drop chance from **0.2% → 0.3%** (×1.5, `acc_clover_mult`) **and** add a **0.5%/kill** chance to drop a random power-up (full_ammo / insta_kill / double_points / nuke), bypassing the per-round cap, **and boost mystery-box top-tier odds** (~+1% per S+/S gun, pulled from the common tier). Works in Paradise. | Passive, KILLER only. Per-player. Live dvars `acc_clover_mult` / `acc_clover_powerup_chance` / `acc_clover_box_*`. (user 2026-06-27, retuned 2026-06-29) |
