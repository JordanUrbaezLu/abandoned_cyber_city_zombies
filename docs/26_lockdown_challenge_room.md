...
| `acc_lockdown_challenge_force "<zone>"` | — | Dev console command (polled only while the feature is enabled): force-start a challenge without springing the trap. |

(the per-feature debug/test dvars were removed 2026-07-16 — the `[ldc]` debug banners now ride the one `acc_dev` flag; see docs/22 §E + memory `debug-banners-gated-by-acc-dev-only`)
