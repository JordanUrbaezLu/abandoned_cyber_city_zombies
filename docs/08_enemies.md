...
- **Spawn**: **always the Plaza** (`acc_box_plaza`; nudge with `acc_protector_spawn_*`), deliberately ~600u
  ...
  player** in the arena.
- **HP (the MIDDLE tier)**: shared 65k/anchor-5 scale with exponent **1.09** (`acc_protector_hp_exp`; the ladder
  is Brutus 1.12 > **Rogue Protector = Panzer 1.09** > Phantom/Avogadro 1.06) × the log co-op multiplier. Solo
  (anchor 5) r5 65k → r10 100k → r20 237k → r30 561k → r40 1.33M.
...
- **Presentation**: a self-contained **slam-down entrance** (ground-tell telegraph → quake + landing FX + a
  ...
  **sprint-lock chase** (`hunt_players` re-pins the goal every 0.5s), the **3D over-head nameplate** + health bar,
