...
5. **Data Shards go to the killer.** Killing a Shielded elite auto-grants the killing player 2 Data Shards on death — there is no world pickup to collect. (reconciled to code 2026-07-11)
...
- **Attacks**: stock mechz BT — flamethrower sweeps, 115-grenade volleys, and a **very hard melee**
  ...
  dead value was an instadown 150). **Flamethrower +20% (user 2026-07-11):** stock burn constants
  ...
  `is_burning` mutex + Jugg branch) — per-tick damage × `acc_panzer_flame_mult` (default **1.2** →
  36 / 24 with Jugg). Claw-grapple is **deliberately OFF** (broken-as-decompiled +
...
- **Hit-location damage (user 2026-07-11 — why he read as "no headshot weakness" + "some guns can't
  headshot"):** he does NOT use the normal-zombie damage path; he runs the stock
  `MechzServerUtils::mechzDamageCallback`, which scales by body part — **head = 100%, exposed power
  core = 100%, everything else (torso/limbs) = stock 10%**, explosives = 50%. Two problems fell out of
  that, both fixed in a wrapper on `actor_damage_func` (`acc_mechz_damage_wrap`, bullet-only — splash/
  explosive/projectile MODs keep their dedicated stock handling):
  ...
  - **The head was gated behind the helmet, so weak guns couldn't headshot at all.** Stock only lets the
    ...
    1500+, scaling toward a 16000 cap — vs stock's **50**), so a non-PaP gun (e.g. the Grav) can never
    break the helmet to expose the head. It read as "only PaP'd guns can headshot" — really it's the
    helmet, not PaP. The wrapper now makes the head a weak spot for **every bullet, faceplate or not**:
    a face hit is the engine `"head"` hitLoc **or** proximity (`acc_panzer_head_radius`, default 21u —
    18 → 21 (+~15%) same day, headshots felt too tight, height-guarded) to the `j_faceplate` tag, and
    it pays **`acc_panzer_head_scale` (default 1.25× base
    ≈ 3.6× a body shot)**. Stock still chips the faceplate as a side effect, so the helmet visually
    breaks — it's just no longer a hard gate on landing headshots.
  - **Dvars:** `acc_panzer_body_scale` 0.35 (set 0.1 = exact stock body), `acc_panzer_head_scale` 1.25,
    `acc_panzer_head_radius` 21. Panzer is the only mechz on the map, so this touches nothing else.
