...
- **10 items** in the drop pool: Gas Tank, Loot Stash, Repair Kit, Rocket Shield, Phase Serum, Boots, Lucky Horseshoe, **Turbocharger (Havoc)** (a hyper-niche Havoc-only mod, added 2026-07-07), **Nuclear Energy** (+15% explosive & energy damage, added 2026-07-07), and **Battery** (boss zaps absorbed → +20% speed surge for 5 s + a blue-green screen aura instead of the slow, 10 s cooldown — buffed from 12 s 2026-07-09, added 2026-07-08). The two tactical grenades (Li'l Arnie + Monkey Bomb) were moved to mystery-box rolls 2026-06-24 — see the note under "The items". (Pool + slot count are live in `_acc_boss_items.gsc`: `init()` logs `pool=10`, `ACC_ITEM_SLOTS_PER_PLAYER 3`.)
...
2. **Enable it at an Implant Bench** — the Plaza bench is in the gated **Implant Lab** side-room off the Plaza spawn (buy the tight-entrance door, `enter_implant`, **1500**, to reach it; 2026-06-26). **Three pads** (2 → 3, 2026-07-09; the row shifted 40u west so the 3rd pad clears the lab's east wall — the room itself needed NO enlargement), Slot 1 / 2 / 3. Hold **ⓕ Use** on a pad to **implant** the carried item into that slot → buff goes active (HUD: `IMPLANT 1 <item>` / `IMPLANT 2 <item>` / `IMPLANT 3 <item>`). A confirmation sound plays. Paradise has its own 3-pad row (south-west, by the Mystery Box).
...
> Horseshoe / Turbocharger / Nuclear Energy / Battery) were added later, so the live pool is **10** (IDs 1–10).
...
| 3 | **Repair Kit** | first-aid box (`p7_spl_first_aid_box` — T7 Assets carve 2026-07-08; was the carpenter power-up icon) | **+13 HP/sec** passive health regen (user 2026-07-07: +3, was 10; `ACC_OVERCHARGE_REGEN 13`) | Passive (caps at max health; pauses while downed). |
...
| 5 | **Phase Serum** | MOTD surgical vial (`p7_zm_mob_vial_surgical_lrg` — T7 Assets carve 2026-07-08; was the generic perk bottle) | **Glitch-suppression aura** (user 2026-06-29 nerf, was a cloak) — any **Glitch Stalker within ~350u is slowed to 1/5 speed AND loses its blink** (its glitch ability). It can still SEE + chase you — just nullified, not blinded. Regular horde unaffected. | Passive. Dvars `acc_phase_serum_radius` (350) / `acc_phase_serum_slow` (0.2). Logic in `_acc_boss_glitch::acc_serum_suppressed`. |
...
| `acc_bench_off_x` | -40 | bench X offset from the Plaza spawn struct (was 0 = centred; -40 since the 3rd pad, 2026-07-09, so the widened row clears the lab's east wall) |
| `acc_bench_off_y` | -350 | bench Y offset from the spawn struct — pushes the row SOUTH toward the back wall, out of the open middle (user 2026-06-24) |
| `acc_bench_off_z` | -35 | bench Z offset from the Plaza spawn struct (it sat too high; 2026-06-18) |
| `acc_bench_pad_sep` | 80 | half-spacing of the bench-pad row along **X** (3 pads at −2·sep / 0 / +2·sep = 160 apart, a row parallel to the south wall) |
...
- **Bench (three pads):** `spawn_bench()` polls the `player_respawn_point` struct, then spawns **three** pads via `spawn_bench_pad(org, slot)` at `base + (-2·sep, 0, 0)` / `base` / `base + (2·sep, 0, 0)` (Slot 1 west / Slot 2 center / Slot 3 east — the east pad is the 2026-07-09 addition). Each pad is a `script_model` + `trigger_radius_use` with `acc_bench_slot` = its fixed target index. The row sits behind the Plaza spawn points against the south wall, out of the open central training area (user 2026-06-24), laid along X (`acc_bench_pad_sep` 80 → 160 apart, `acc_bench_pad_radius` 40) so their use-volumes don't overlap. `bench_use_loop()` reads its pad's slot: `equip_slot(player, slot, carried)` (which `unequip_slot`s the old occupant first), free into an empty slot or `ACC_BENCH_SWAP_COST` (2500) to replace a full one, plays `acc_item_implant`, and clears the carry.
...
- **Glitch suppression (Phase Serum):** `apply_arnie_cloak` sets a custom `player.acc_cloak_glitch` flag — deliberately **NOT** `zm_utility::increment_ignoreme` (that hid you from the *entire* horde = invulnerable, the rejected bug). The Glitch Stalker honors it by setting **`host.closest_player_override = &glitch_pick_uncloaked_target`** on every spawn: stock `get_closest_valid_player` consults this per-AI picker FIRST (`_zm_utility.gsc:1472`) to derive BOTH the Stalker's movement target (`favoriteenemy`) and its melee target (`enemy`), so the suppression hides you from its **whole behavior** — follow + melee + blink + charge — not just the two blink/charge calls (the old scope, the "Phase Serum doesn't work" bug). The picker strips suppressed players then delegates to the stock factory target picker so everyone else is targeted exactly as stock; the regular horde always sees you.
...
- A 3-slot inventory forces trade-offs. 10 items across several distinct build axes (mobility / economy / survival / weapon-mod / anti-boss) mean the three you wear say a lot about your build intent.
- Wearing them all would remove the decision.
- With 10 items in the pool and 3 slots, **there are C(10,3) = 120 possible equipped triples** per run. Meaningful run-to-run combinatorial variance on top of drop RNG.
...
- With 10 items in the pool and ~1 drop per boss, all three slots fill over the first few boss rounds and implanted-duplicate refusals leave later drops for teammates.
...
[`_acc_boss_items.gsc`](../scripts/zm/zm_abandoned_cyber_city/_acc_boss_items.gsc): all 10 pool
...
| Data Cache (3 plaza + 2 pit) | `p7_zm_sta_computer_tower_01` — Gorod computer tower (24×30×72) | origin mesh-centered → +36 z lift |
