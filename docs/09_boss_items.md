...
- **Duplicates** (user 2026-07-08 rework): an item you already have **IMPLANTED** (any slot) **cannot be
  ...
  **STACKABLE exception (user 2026-07-25): the Sentry Drone is the ONLY item that CAN be implanted in more
  than one slot** (up to 3 copies = up to 3 drones orbiting; each copy adds one). The item table's
...
| 5 | **Phase Serum** | MOTD surgical vial (`p7_zm_mob_vial_surgical_lrg` — T7 Assets carve 2026-07-08; was the generic perk bottle) | **Phase-boss suppression aura** (user 2026-06-29 nerf, was a cloak; Phantom added 2026-07-11; **whole item −20% 2026-07-22** — the 0.2 rate "basically freezes glitches") — any **Glitch Stalker within ~350u is slowed to 0.36× speed AND loses its blink** (its glitch ability; was 1/5), and any **Phantom in the same aura is slowed by 24%** (was 30%; milder: its gait only — teleports keep working). Both can still SEE + chase you — just hindered, not blinded. Regular horde unaffected. | Passive. Dvars `acc_phase_serum_radius` (350) / `acc_phase_serum_slow` (0.36, Glitch) / `acc_phantom_serum_slow` (0.76, Phantom). Aura check shared in `acc_utility::serum_aura_active`; consumers `_acc_boss_glitch::acc_serum_suppressed` + `_acc_boss_phantom::phantom_speed_think`. |
...
| `acc_drone_orbit_radius` | 72 | Sentry Drone **orbit** radius around the owner (u). WORLD-space ring — it circles you independent of your facing so you can turn and watch it come around (2026-07-24, replaced the old shoulder-locked offset that stayed glued behind your view). The 2026-07-25 random ROAM wanders 0.45×–1.25× of this |
| `acc_drone_orbit_speed` | 65 | Sentry Drone orbit angular speed (degrees/sec; ~5.5 s a full lap). The random ROAM wanders the pace 0.4×–1.7× of this (forward-only) |
...
| `acc_drone_fire_sec` | 0.9 | Sentry Drone **primary bolt** cadence (s) |
...
| `acc_drone_launch_sec` | 4.5 | Sentry Drone **rocket launcher** (secondary) cadence (s) — a real `s1_mahem` rocket at the farthest target |
...
| `acc_drone_swarm_mult` | 1.5 | **SENTRY SWARM** set bonus (all 3 slots = Sentry Drone): multiplier on every drone's damage, orbit roam speed, and fire rate (floor-clamped at 0.25 so a bad value can't zero a wait) |
