...
- **15 items** in the drop pool: **Sentry Drone** (the pool's SUPPORT-CHARACTER implant — an always-on hovering Maxis-Drone helper that free-roams around you (2026-08-01 movement rework) and zaps the nearest zombie in your radius; **REPLACED Gas Tank 2026-07-22**, reusing num 1 because the `acc_implants` nibble is full at 15), Loot Stash, Repair Kit (**+8 HP/s** — nerfed 20% from 10 on 2026-07-22), Rocket Shield, Phase Serum (**−15% 2026-08-03 on top of the −20% 2026-07-22** — glitch 0.456× speed, phantom −20.4%), Boots (**+10% move** — buffed from 8% 2026-07-14), Lucky Horseshoe, **Turbocharger (Havoc)** (a hyper-niche Havoc-only mod, added 2026-07-07), **Plasma Generator** (+10% energy-weapon damage, added 2026-07-14), **Battery** (boss zaps absorbed → **+24%** speed surge for 5 s + a blue-green screen aura instead of the slow, 10 s cooldown — surge buffed 20% from +20% on 2026-07-22; cd buffed from 12 s 2026-07-09, added 2026-07-08), **Berzerker** (+35% melee swing speed at 5% max HP per connecting melee — regular knife bash / Leviathan Axe / Action Figure / Ballistic Knife stab, added 2026-07-11), **High Caliber Rounds** (+25% BULLET-gun damage — the ballistic counterpart, pushes builds toward bullet guns; excludes energy weapons, added 2026-07-14), and **Warhead Bomber** (+20% explosive damage, added 2026-07-14), and **Hive Node** (the pool's co-op **SUPPORT beacon** — a heal+shield aura *plus* a double-tap-jump "Bloom" burst that full-heals / ranged-revives / shields the whole squad, with a reward-the-medic points+shards loop so a support player is paid for the slot; added 2026-07-16). **Plasma Generator + Warhead Bomber are the 2026-07-14 SPLIT of the old "Nuclear Energy" item** (which was +10% to explosive OR energy) into two separately-tunable implants. The two tactical grenades (Li'l Arnie + Monkey Bomb) were moved to mystery-box rolls 2026-06-24 — see the note under "The items". (Pool + slot count are live in `_acc_boss_items.gsc`: `init()` logs the live `pool=` size, `ACC_ITEM_SLOTS_PER_PLAYER 3`.)
...
- **Duplicates** (user 2026-07-08 rework): an item you already have **IMPLANTED** (any slot) **cannot be
  ...
  **STACKABLE exception (user 2026-07-25): the Sentry Drone is the ONLY item that CAN be implanted in more
  than one slot** (up to 3 copies = up to 3 drones roaming around you; each copy adds one). The item table's
  `.stackable` flag stands every duplicate gate down for it — ground grab, `equip_slot`, the bench (a
  pointless same-slot self-replace is still refused: "stack it on another pad"), the vault withdraw
  (`_acc_transfer`), and the carried HUD nibble (a carried-while-implanted copy shows). A carried-dupe grab
  is refused-and-left (never shard-converted — the copy is usable, you just can't carry two at once), and
  the drop-old-carry path drops a carried Sentry back to the ground instead of silently discarding it.
  Grep `item_is_stackable` for all 7 touchpoints; the count-based fleet lives in `apply_sentry_drone` /
  `remove_sentry_drone` / `sentry_drone_guardian`.
- **Drops always land on the floor** (root-caused 2026-07-08 after "still floating — the glow is airborne
  ...
  — **except tagged loot-pickup models** (`.acc_item_id` / `.acc_shard_count`), stepped through like bodies
  (2026-08-03 fix: the swap re-drop traced onto the still-standing grabbed pickup's model top, so each
  drop-and-swap raised the item by ~`z_lift`+model height — compounding "climbs into the sky" drift) —
...
|----|------|-------|------|-----------|
| 1 | **Sentry Drone** (REPLACED **Gas Tank** 2026-07-22 — the `acc_implants` nibble is full at 15, so the new implant reused num 1; the Gas Tank helpers stay dormant in `_acc_boss_items.gsc`, kinetic-battery precedent) | Origins **Maxis Drone chassis** (`veh_t7_dlc_zm_quadrotor_piece_body` — the LOD0-clean STATIC buildable-piece variant, T7 Assets carve 2026-07-22 `acc_t7_drone.gdt`, source_map `p7_zm_tomb`, 35.8×8.7×11.7u; **never** the assembled `veh_t7_dlc_zm_quadrotor`, rigged at all 8 LODs. ×1.5 pickup / ×1.3 flying body) | **Always-on helper drone that FREE-ROAMS around you** (**motion rework 2026-07-24** — user "pretty stiff … stuck behind my player, I cant look at it": the old fixed right-shoulder offset was locked to your yaw and stayed glued behind wherever you looked; since then the anchor lives on a **world-space circle** around you [`acc_drone_orbit_radius` 72u] independent of your facing — turn and it's right there. **FREE ROAM (2026-08-01 rework, user "it just circles you… make it more free roaming around the player and bob in height" — replaces the 2026-07-25 forward-only ring roam):** every 1.2–3.2 s the drone picks a NEW random **bearing goal** (signed ±170°, so it reverses and cuts chords across the circle instead of lapping), a radius fraction (0.35×–1.25× of the ring dvar) and a height offset, steering the bearing shortest-path rate-limited to `acc_drone_orbit_speed` ×1.6 °/s while radius/height ease softly — swings in tight, drifts wide, doubles back, never traces the same path twice; the old "passes through your view every lap" guarantee is now statistical (random bearings cross it constantly); per-drone state so stacked drones desync naturally; the ring dvars still scale everything. Height is a **layered bob**: a fast ~2.25 s ±6u ripple riding a slow ~6.8 s ±10u swell, phase-offset per drone, with a 30u z-floor above your feet so it never scrapes your knees in tight rooms. Still a 20 Hz MoveTo chase + eye-trace wall clamp + 500u leash snap; script-flown `script_model`, NOT an AI actor — no navmesh/BT/actor-pool cost, and the ally BT can't fire on this map anyway). **PRIMARY FIRE — a VISIBLE plasma bolt** (user 2026-07-24 "it doesnt shoot an actual projectile so i cant see its shots") at the best target every **1.2 s** (2026-08-01 −25%-DPS pass: cadence 0.9 → 1.2 s on every damage lane, per-hit numbers untouched): a blue geotrail script-mover flies drone→target (**reuses the Triple Take `acc_ttk_bolt_fx` scriptmover CF, value 1 — ZERO new bits, the pool is FULL**), 700u owner-centered range (2D + 320u z-band), drone-eye LOS, damage lands on ARRIVAL = **max(168, ~19.1% max HP)** round-proof (2026-07-27 −15% then −10%, was max(220, 25%)). **TARGETS BOSSES + ELITES TOO, and PREFERS them (user 2026-07-25 "these drones need to target the bosses and elites"):** the nearest tough target (`sentry_is_tough` — boss/mini-boss/elite/shielded/Glitch/Fury) beats any normal zombie, and tough targets ignore the sibling stack-claims (all drones may focus one boss). Tough hits ride a separate **CHIP damage lane** — bolt = **~26.8% of ONE normal zombie's round HP** (`acc_drone_boss_frac` 0.26775; was 35% pre-2026-07-27), launcher blast = **~61.2%** (`acc_drone_launch_boss_frac` 0.612; was 80%), the cyberjack `storm_dot_tick` recipe — NEVER a fraction of the boss's own pool (the exact mark bypasses the boss per-hit cap by design; 25%-of-a-boss's-maxhealth bolts would melt the roster). **SECONDARY FIRE — a ROCKET LAUNCHER** (user 2026-07-24 "give it a launcher as a secondary firing projectile") every **6.0 s** (2026-08-01, was 4.5 s — the same −25% cadence pass): a **real, VISIBLE `s1_mahem` rocket** (its own model/smoke-trail/explosion/boom — the `_acc_civil_protector::mahem_shot` recipe) `MagicBullet`-fired at the **FARTHEST** qualifying zombie (beyond `acc_drone_launch_min_range` 260u), and the **returned missile entity is TRACKED** (`sentry_rocket_watch`) so the **round-proof scripted exact-mark AoE** [`acc_drone_launch_radius` 200u, max(306, ~45.9% max HP) — 2026-07-27 nerf, was max(400, 60%)] — the real kill — lands at the rocket's **actual detonation point/time** (a crossing zombie can blow it early; a fixed timer at the aim point would deal invisible "ghost" damage away from the visible explosion). **OWNER SAFETY (adversarial-review catch, same day):** a self-attributed rocket splash passes stock's self-damage MOD whitelist (`_zm.gsc:1435`) at s1_mahem's raw ~3100 — the RP mahem cap is attacker-gated and never applies to a player attacker — so the watch holds a rolling `acc_drone_rocket_until` window on the owner and `_acc_elites::on_player_damaged` **zeroes self-`mahem` hits inside it** (window-gated so a hand-fired box Mahem outside the window keeps its normal self-splash risk; teammates were never at risk — stock "players can't hurt each other", `_zm.gsc:1427`; the scripted AoE is zombie-only by construction). Both weapons deal **player-attributed exact-mark DoDamage** (cyberjack `storm_one_shot` recipe) so kills pay YOUR points/drops/Loot-Stash/leaderboard. Bolt impact arc = `acc_cj_shock` tag-FX + `acc_cj_zap` SFX; rocket FX = the native `s1_mahem` explosion (no new FX/CF registrations — both pools are full). Pauses (no cover fire) while you're down; respawns after your death via the guardian loop; deleted on unequip/bleed-out wipe so nothing outlives the implant. | Passive — it just guards you. **STACKABLE (user 2026-07-25): the ONLY implant legal in more than one slot** — implant up to 3 copies (one per bench pad; the vault + ground grabs allow duplicates for this id only) for up to **3 drones roaming at evenly-spread starting bearings** (2 = opposite sides, 3 = a triangle; the guardian re-spreads on any roster change — since the 2026-08-01 free-roam rework the re-spread also resets each drone's bearing goal to its new slot, so they fan out then wander independently). Stacked drones **fan out their bolts** via a 0.7 s target claim (siblings prefer unclaimed zombies; solo cadence unchanged) — rockets don't claim (overlapping AoE is fine). **SENTRY SWARM set bonus (easter egg, same day): ALL 3 slots = Sentry → ×1.5 damage (bolt + rocket blast), roam steer speed, and fire rate on every drone** (post-2026-08-01 swarm cadences: bolt 1.2→0.8 s, rocket 6.0→4.0 s — the ×1.5 ratio rode through the recadence unchanged) (`acc_drone_swarm_mult` 1.5; count read live, so the bonus engages the moment the 3rd copy goes in — the `SENTRY SWARM` reveal print is the only in-game advertisement, that discovery IS the egg; breaking the stack powers back down with a print). Code: the SENTRY DRONE section in `_acc_boss_items.gsc` (`sentry_hover_loop` [free-roam hover] / `sentry_fire_loop` + `sentry_bolt_flight` [primary bolt] / `sentry_launcher_loop` + `sentry_launch_rocket` + `sentry_rocket_watch` + `sentry_launch_blast` [launcher] / `sentry_apply_hit` [shared exact-mark hit]) + the self-mahem negation lane in `_acc_elites::on_player_damaged`. Live dvars `acc_drone_orbit_radius`/`_speed`/`acc_drone_hover_z`, `acc_drone_range`/`_fire_sec`/`_bolt_speed`/`_damage`/`_maxhp_frac`/`_boss_frac`, `acc_drone_launch_sec`/`_min_range`/`_radius`/`_damage`/`_maxhp_frac`/`_launch_boss_frac`. |
...
| `acc_gas_dtap_ms` | 350 | *(DORMANT 2026-07-22 — Gas Tank retired; helpers stay in-file)* Gas Tank double-tap-sprint window (ms) |
| `acc_gas_burst_mult` | 2.0 | *(DORMANT)* Gas Tank nitro burst move-speed multiplier (+100%) |
| `acc_gas_regen_sec` | 60 | *(DORMANT)* Gas Tank cooldown/regen seconds after the 5 s burst (= NITRO bar refill time) |
| `acc_drone_orbit_radius` | 72 | Sentry Drone **roam-circle** radius around the owner (u). WORLD-space — moves independent of your facing (2026-07-24, replaced the old shoulder-locked offset that stayed glued behind your view). Since the 2026-08-01 **FREE-ROAM** rework the drone no longer laps a ring: every 1.2–3.2 s it picks a new random bearing goal (signed ±170° — it reverses and cuts chords across the circle) at 0.35×–1.25× of this radius plus a height offset, easing between goals, with a layered height bob (fast ~2.25 s ±6u ripple over a slow ~6.8 s ±10u swell, per-drone phase) and a 30u z-floor above the owner's feet. (The 2026-07-25 forward-only ring roam wandered 0.45×–1.25×) |
| `acc_drone_orbit_speed` | 65 | Sentry Drone bearing-steer speed BASE (degrees/sec). Free-roam rework 2026-08-01: no longer a constant lap rate — the drone pursues its current bearing goal shortest-path, rate-limited to ×1.6 of this, so pace is implied by angular distance (near the goal it lingers). (Was a fixed lap ~5.5 s, with the 2026-07-25 roam wandering the pace 0.4×–1.7×, forward-only) |
| `acc_drone_hover_z` | 58 | Sentry Drone hover height above the owner's feet (u) |
| `acc_drone_range` | 700 | Sentry Drone owner-centered target radius (2D, u; 320u z-band hardcoded) — shared by the bolt + launcher |
| `acc_drone_fire_sec` | 1.2 | Sentry Drone **primary bolt** cadence (s) (2026-08-01 −25%-DPS pass: 0.9 → 1.2; per-hit damage dvars untouched) |
| `acc_drone_bolt_speed` | 4000 | Sentry Drone primary bolt visual speed (u/s; min-flight 0.25 s so the CF snapshot renders — Avogadro rule) |
| `acc_drone_damage` | 168 | Sentry Drone flat damage floor per bolt (2026-07-27 −15% then −10%, was 220) |
| `acc_drone_maxhp_frac` | 0.19125 | Sentry Drone round-proof bolt damage: fraction of the target's max HP if larger than the flat floor (~6 bolts a kill at any round; ×0.765 of 0.25) — NORMAL zombies only (tough targets ride `acc_drone_boss_frac`) |
| `acc_drone_boss_frac` | 0.26775 | Sentry Drone bolt **CHIP lane vs bosses/elites** (2026-07-25 "target the bosses and elites"): fraction of ONE normal zombie's round HP per bolt (`level.zombie_health` base, the `storm_dot_tick` recipe) — never a fraction of the boss's own pool. ×0.765 of 0.35 (2026-07-27 −15% then −10%) |
| `acc_drone_launch_sec` | 6.0 | Sentry Drone **rocket launcher** (secondary) cadence (s) — a real `s1_mahem` rocket at the farthest target (2026-08-01 −25%-DPS pass: 4.5 → 6.0) |
| `acc_drone_launch_min_range` | 260 | Launcher min distance from the OWNER a target must clear to be fired at — the rocket is the long-range weapon, the bolt owns close range. (NOT a self-splash guard: a crossing zombie can still detonate the rocket near you — the owner is protected by the `_acc_elites` self-mahem negation lane instead) |
| `acc_drone_launch_radius` | 200 | Launcher round-proof AoE radius at the impact point (u) |
| `acc_drone_launch_damage` | 306 | Launcher flat damage floor per blasted zombie (heavier than the bolt; ×0.765 of 400, 2026-07-27 −15% then −10%) |
| `acc_drone_launch_maxhp_frac` | 0.459 | Launcher round-proof AoE: fraction of a zombie's max HP if larger than the flat floor (×0.765 of 0.6) — NORMAL zombies only (tough targets ride `acc_drone_launch_boss_frac`) |
| `acc_drone_launch_boss_frac` | 0.612 | Launcher blast **CHIP lane vs bosses/elites** (2026-07-25): fraction of ONE normal zombie's round HP per blasted tough enemy (~2.3× the bolt chip, ratio preserved). ×0.765 of 0.8 (2026-07-27 −15% then −10%) |
| `acc_drone_swarm_mult` | 1.5 | **SENTRY SWARM** set bonus (all 3 slots = Sentry Drone): multiplier on every drone's damage, roam steer speed, and fire rate (floor-clamped at 0.25 so a bad value can't zero a wait; post-2026-08-01 swarm cadences = bolt 0.8 s / rocket 4.0 s, ratio preserved) |
...
| `acc_battery_boost_mult` | 1.24 | Battery item surge multiplier (+24% move; user 2026-07-22 +20% buffed by 20%) when a boss zap is absorbed |
...
- The **stackable Sentry Drone** (2026-07-25) adds the multi-copy builds on top (double/triple-drone is a legitimate all-in support build that spends 2-3 slots on the same axis) — it widens the triple space rather than shrinking the trade-off, since every stacked copy is a slot NOT spent on mobility/damage/survival.
