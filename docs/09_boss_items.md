...
### In-game effect blurbs (`desc`, 2026-07-11)

Before 2026-07-11 **no UI surface said what an item actually does** — only `id - name` (effects lived
only in this doc). Every pool item now carries a one-line `desc` in `build_item_pool()` shown on:
the **pickup hint** (`Hold ⓕ to grab 7 - Lucky Horseshoe - Luckier drops and box rolls` — so you know
what it does BEFORE grabbing), the **grab toast**, the **bench implant toast**, the **Transfer Vault
withdraw toast**, and appended to the persistent **`IMPLANT N` HUD lines** (so you can always see what
your implants are doing). Rules for writing a `desc` (enforced by the comment on `item()`):

1. **Vague wording per docs/31** — direction only, NO magnitudes (`Faster melee but drains some
   health`, never `+35% melee speed`). Exact values stay in the table above.
...
3. **Short** (~30 chars) — it shares a line with `IMPLANT N id - name` on the left HUD stack.
...
- **Intentionally synergistic combos** (all reference live items):
  ...
  - **Nuclear Energy + an energy/explosive build** — +15% on Havoc/Tac-19/AE4/RW1/Blast-O-Matic/CEL-3/Thundergun/launchers stacks with the Mega Flopper explosive layer. **The Triple Take (2026-07-11) is the implant's signature gun:** the +15% lifts its 3-bolt per-trigger to at/slightly above the MORS per-shot at every PaP tier (docs/04). **The CEL-3 Cauterizer (AW directed-energy spread SG) joined the energy list 2026-07-11** — a modest synergy on a B-tier gun; the Peacekeeper stays a ballistic (non-energy) shotgun by design.
