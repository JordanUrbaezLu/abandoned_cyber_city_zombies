...
> **Current working-tree state (user 2026-07-10 "hardcode code[=god] mode and dev mode on"):** the resolver has
> an **ACTIVE** `level.acc_dev = true;` line *and* an `level.acc_god = true;` line hardcoded directly under the
> dvar-gate lines ([`zm_abandoned_cyber_city.gsc:376`](../scripts/zm/zm_abandoned_cyber_city.gsc#L376) /
> [:417](../scripts/zm/zm_abandoned_cyber_city.gsc#L417)) — so a plain build currently runs dev **and** god
> ON no matter what the dvar says. **All on-screen dev debug UI was stripped the same day** (user: "remove all
