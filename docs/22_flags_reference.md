...
> 2026-07-16** (user: only `acc_dev`/`acc_god`/mock flags exist). **Dev boss cadence since 2026-07-17: 2 Glitches/round + a round-3 Phantom; since 2026-07-24 also the ROUND-4 ALL-BOSS WAVE — all 5 bosses at once (`boss_count` dev branch returns 4 → the no-dup deck deals one of each roster type, + a Trench Warden branch in `_acc_boss::round_hook_loop` that bypasses the power gate/round-5 floor one-shot). All hardcoded dev branches; Brutus + roster otherwise REAL** like
...
| `acc_boss_nameplate_on` | **`1` (on)** | Boss 3D over-head NAME plate (grandfathered 2026-07-02 A/B presentation lever; documented 2026-07-24). `0` = no plates. Since 2026-07-24 the plate is NAME-ONLY, and because the LUI bar rows are fed from the same `attach()` choke point, `0` also suppresses the **boss health-bar rows** — it's the "no boss HUD at all" lever. The rows' own kill-switch is the hardcoded **`level.acc_boss_bars_lui`** bool (`_acc_boss_nameplate.gsc __init__`, the `level.acc_aetherium_hud` pattern — flip + rebuild, never a dvar). | [`_acc_boss_nameplate.gsc` `attach`](../scripts/zm/zm_abandoned_cyber_city/_acc_boss_nameplate.gsc) |
| `acc_boss_bar_2d` | `0` | The RETIRED 2026-07-02 2D top-screen hudelem boss bar, kept fully wired for A/B (grandfathered; documented 2026-07-24). `1` = re-enable it live. Superseded twice over by the LUI `CoD.AccBossBars` rows — only useful for comparing renderers. | [`_acc_health_bars.gsc` `boss_bar_listener`](../scripts/zm/zm_abandoned_cyber_city/_acc_health_bars.gsc) |
...
| `acc_gas_dtap_ms` | `350` | *(DORMANT 2026-07-22 — Gas Tank retired, replaced by the Sentry Drone at pool num 1; the gas helpers stay in-file.)* Gas Tank: max ms between two **Sprint-button** presses (`SprintButtonPressed`) to count as a double-tap (triggers the nitro burst). Higher = more forgiving. Read live. | [`_acc_boss_items.gsc` `gas_tank_watch`](../scripts/zm/zm_abandoned_cyber_city/_acc_boss_items.gsc) |
| `acc_gas_burst_mult` | `2.0` | *(DORMANT)* Gas Tank: nitro-burst move-speed multiplier (**+100%**, i.e. double speed) applied for the 5 s burst. Read live (rides `recompute_move_speed`). | [`_acc_utility.gsc` `recompute_move_speed`](../scripts/zm/zm_abandoned_cyber_city/_acc_utility.gsc) |
| `acc_gas_regen_sec` | `60` | *(DORMANT)* Gas Tank: cooldown/regen seconds after the 5 s burst — you can't re-fire until full. Also the NITRO-bar refill time (the bar reads the same dvar). Read live. | [`_acc_boss_items.gsc` `gas_tank_burst`](../scripts/zm/zm_abandoned_cyber_city/_acc_boss_items.gsc) |
...
| `acc_hive_regen` | `12` | Hive Node: passive aura HP/sec regen granted to every player in range (the carrier included). Nerfed from 15 on 2026-07-22; Repair Kit is 8 (also nerfed 2026-07-22) and self-only. Read live per 0.25 s tick. | [`_acc_boss_items.gsc` `hive_aura_loop`](../scripts/zm/zm_abandoned_cyber_city/_acc_boss_items.gsc) |
...
| `acc_trench_layer_speed_pct` | `4` | Trench: **move-speed bump per layer** (anim-rate ×`(1 + layer·pct/100)`). Layer 1 = +4%, layer 2 = +8%, … L5 = +20% (was 4 → 3.5, user 2026-07-16; 3 → 4, user 2026-08-01). Stacks on top of the round gait/rate; always ≥ 1.0 (no slow-mo). | [`_acc_zombie_speed.gsc` `apply_speed_for_round`](../scripts/zm/zm_abandoned_cyber_city/_acc_zombie_speed.gsc) |
...
| `acc_trench_layer_dmg_add` | `5` | Trench: **flat melee bonus per layer (HP)** — a zombie melee hit you take in layer L adds `L × this` HP. L1 = +5 (≈50/hit), L2 = +10 (≈55), … L5 = +25 (≈70) (was 6 → 5, user 2026-07-16; 4 → 5, user 2026-08-01). Added to the **player's incoming damage** (open-field melee ignores `self.meleeDamage`, so this is the only reliable lever). | [`_acc_bus_trench.gsc` `trench_melee_scaled`](../scripts/zm/zm_abandoned_cyber_city/_acc_bus_trench.gsc) |
...
| `acc_trench_income_l1` … `_l5` | `60` / `41` / `27` / `16` / `9` | Trench: **seconds between passive shard grants per layer** — L1 (Bus Station pit) 60s, L2 41s, L3 27s, L4 16s, L5 9s. Deeper = faster (more reward for the greater risk). (Income −25%, user 2026-08-01: was 45/31/20/12/7.) The clock carries across layer changes (paid at the **current** layer's rate), so descending never loses progress. Set a layer's value to `0` to disable income on that layer. | [`_acc_bus_trench.gsc` `trench_income_interval`](../scripts/zm/zm_abandoned_cyber_city/_acc_bus_trench.gsc) |
...
| `acc_rampage_round_bonus` | `7` | **RAMPAGE INDUCER v2** (user 2026-08-03): rounds ADDED to the speed-curve input while the breaker toggle on the Plaza→Alley connector is ON (`level.acc_rampage_on` — set ONLY by the `_acc_rampage` station, toggleable rounds 1–4, **seals at round 5**; the breaker pulses RED while ON). Read live in `rampage_bonus()`/`effective_round()`, consumed by exactly: normal-zombie on-spawn + keep-alive, `glitch_speed_think`, `shielded_speed_think`, and the Scientist's `sprint_pin` (curve + anti-horde floor). No on/off dvar exists by design — the old Inducer's dvar watcher was the "sprints then stops" bug. | [`_acc_zombie_speed.gsc` `effective_round`](../scripts/zm/zm_abandoned_cyber_city/_acc_zombie_speed.gsc) |
...
| `acc_glitch_speed_mult` | `1.005` | Anim rate vs the round's normal zombies — ONE lever drives both chase speed **and melee-swing speed** (whole-ASM playback). (user 2026-07-17: 0.86 → 1.005, cut the 07-16 "they swing too fast" pass in half after it felt "too passive" — now about the same as the horde; the blink is its mobility. 2026-07-16: 1.15 → 0.86.) Locks the horde's gait × this rate. (Rampage-aware since 2026-08-03 — the round input is `effective_round()`, see `acc_rampage_round_bonus` in the zombie-speed table.) |
...
| `acc_phantom_serum_slow` | `0.796` | Gait multiplier while the Phantom is inside a **Phase Serum** holder's aura (radius = the shared `acc_phase_serum_radius`, 350) — **20.4% slow** (user 2026-08-03: item −15%, was 0.76/24%; 2026-07-22: whole item −20%, was 0.7/30%; 2026-07-11 history `0.5` → `0.6` → `0.7`). The Glitch concept at a milder penalty: the Glitch takes `acc_phase_serum_slow` 0.456 (was 0.36 — same −15% retune; 0.2 before the −20% one) **and** loses its blink; the Phantom only slows — **teleports keep working**. Read live each speed sweep (`phantom_speed_think` → `acc_utility::serum_aura_active`). |
...
| **Avogadro "cyberhacker" (user 2026-07-04; UNCAPPED map-roamer since the 2026-07-24 rework)** | | Fast electric harasser: low-damage stun-lock shots + machine hacks. Since the perk scatter he ROAMS THE MAP machine-to-machine and can hack **ANY perk (all 10) + PaP** with **no simultaneous cap** — the 30 s per-hack expiry + pad travel time is the counter-pressure. Spawns in the Lab; HP = the Phantom's (`scale_phantom_hp` × `acc_phantom_hp_exp`). 3rd boss-roster type. |
...
| `acc_avo_rehack_cd_ms` | `45000` | **Re-hack cooldown, level-wide (2026-07-25 annoyance review):** after a hacked machine restores, NO Avogadro may re-hack that perk for this many ms — the player-facing promise that a returned perk stays usable for a window. Without it, uncapped hacking let him camp one machine (or ping-pong the jukebox room's two pads) into a near-permanent blackout. Stamped in `undo_hack`, enforced in `is_key_enabled`. |
| `acc_avo_anim_rate` | `1.2` | Anim playback rate applied at spawn via `ASMSetAnimationRate` — run gait at boosted playback; also speeds his throw anim. Playtest ladder (user 2026-07-06): 1.0 too slow → 2.0 way too fast → 1.5 still out-speeds the player → 1.2 → 1.15 shipped. **Raised ONE proven rung to `1.2` in the 2026-07-24 rework** (user: "he also moves faster" **+ "be careful with the speed lever — it's super sensitive"** — 0.05 steps were felt in playtests, so no leaping; the catch-crossover sits somewhere in (1.2, 1.5)). Read at SPAWN — set it live and the next Avogadro picks it up. Persists (the `_acc_zombie_speed` mechanism); safe because it does NOT touch the run-cycle override that froze Brutus. |
