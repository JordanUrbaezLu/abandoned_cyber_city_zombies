...
> also `SetDvar`s the surviving sub-dvars (`acc_open_map`, `acc_glitch_test`, `acc_variants_debug`,
> `acc_hudelem_debug`) off that one value. **To add a dev behavior: branch on `IS_TRUE( level.acc_dev )`
> and hardcode the value, or add a `SetDvar` line in the resolver — never introduce a new toggle.**
>
...
> **Current working-tree state (user 2026-07-09 test cycle "hardcode on for both"):** the resolver has an
> **ACTIVE** `level.acc_dev = true;` line *and* an `level.acc_god = true;` line hardcoded directly under the
> dvar-gate lines ([`zm_abandoned_cyber_city.gsc:376`](../scripts/zm/zm_abandoned_cyber_city.gsc#L376) /
> [:412](../scripts/zm/zm_abandoned_cyber_city.gsc#L412)) — so a plain build currently runs dev **and** god
> ON no matter what the dvar says. `prep_release.ps1` **fails** on these lines; **comment them out before any
...
|---|---|---|
| `acc_open_doors 1` | Set every `enter_*` buyable-door flag → the whole map walkable (the manual open-everything bypass; also clears the door slabs in place). | [`_acc_dev.gsc:128`](../scripts/zm/zm_abandoned_cyber_city/_acc_dev.gsc#L128) |
| `acc_skip_round 1` | Kill all live zombies + end the round → advance to the next round. | [`_acc_dev.gsc:242`](../scripts/zm/zm_abandoned_cyber_city/_acc_dev.gsc#L242) |
| `acc_tp_perks 1` | Teleport all players to the perk row. | [`_acc_dev.gsc:118`](../scripts/zm/zm_abandoned_cyber_city/_acc_dev.gsc#L118) |
| `acc_tp_spawn 1` | Teleport all players back to spawn. | [`_acc_dev.gsc:123`](../scripts/zm/zm_abandoned_cyber_city/_acc_dev.gsc#L123) |
...
| `acc_armory_rack_max` | **`8`** | **The Armory** (upper room, [docs/39](39_armory.md)): max weapons the shared **team weapon rack** holds (deposit refused when full). | [`_acc_armory.gsc` `deposit_gun`](../scripts/zm/zm_abandoned_cyber_city/_acc_armory.gsc) |
...
| `acc_avo_bolt_speed` | `1100` | Bolt projectile speed (units/sec) — dodgeable at range. |
...
| `acc_avo_slow_mult` | `0.70` | Move-speed multiplier his **shot** applies — **−30%**, matches the Phantom/Rogue (user 2026-07-05, was `0.75`). Read by `recompute_move_speed` while `acc_avogadro_slowed`. Mega EC softens to `acc_boss_slow_mega_mult` (−10%). |
...
> **`acc_dev` is the master switch for ALL on-screen debug channels (user 2026-07-05).** Every
> `*_debug` channel below — plus the boss traces (`acc_phantom_debug`, `acc_avo_debug`,
> `acc_warden_debug`), `acc_dmg_debug`, `acc_hudelem_debug`, `acc_crash_debug`,
> `acc_perk_lights_debug`, `acc_lockdown_debug`, `acc_lockdown_challenge_debug`,
> `acc_mega_flopper_debug`, and NSZ's `nsz_debug` — is now gated `( IS_TRUE( level.acc_dev ) || <its dvar> )`.
> So **`acc_dev 1` turns them all on** (dev sandbox) and **`acc_dev 0` keeps every one silent** — the
> per-channel dvar remains only as an optional *narrow* filter for a specific channel without full dev
> mode. This is the "one flag" rule: normal play never shows a debug banner, and you never have to
> remember a second dvar. (Five ungated diagnostics that were leaking in normal play — the trench
> `[BRIDGE]` readout, the `[AVO]` spawn steps, and Avogadro `HACKED:`/`RESTORED:` — were gated behind
> `acc_dev` in the same pass.)
