# 22 - Community Techniques Ledger
...
[17_reference_maps_study.md](17_reference_maps_study.md)). Mined 2026-06-12
...
- **For our map:** If we outgrow the cherry hijack for Aura Blast or add a 10th custom perk, this is the shipped-proven full registration path; the unused-specialty list (disarmexplosive/holdbreath/jetpack) is the scarce resource to budget.

### Complete fix list for stock-but-unfinished _zm_perk_electric_cherry (validates our Aura Blast hijack)
...
- **For our map:** Checklist to audit our _acc_perk_aura_blast hijack against — items 5 and 6 are silent-failure traps (wrong jingle/targetname keeps machine bound to staminup; clip-size placeholder breaks reload-attack scaling).
...
- **For our map:** This is our Mega perk upgrade system, shipped: piggyback effects on free stock specialties (no second machine, no HUD plumbing for the buff itself), gate via challenge or Data Shard spend, and use level.perk_lost_func + get_player_perk_purchase_limit hooks we don't currently touch.
...
### CSC client-module pattern: perk clientfields + FX callbacks (Phase 4 blueprint)
...
- **For our map:** Our Phase 4 client modules (.csc for perk FX, boss FX, Cyberware visuals) should copy this exact mirrored-registration shape; it also validates our cherry-hijack since this is the community-fixed cherry our aura-blast pipeline descends from.
...
- **For our map:** Mostly adopted via our aura-blast/cherry hijack, but register_perk_machine_power_override + the gsh-tunables layout are the clean path for Mega perk tier variants with their own machines.
...
- **For our map:** Aura Blast can be a first-class perk under its own specialty (e.g. specialty_phdflopper or specialty_fireproof if cherry is kept for a real Electric Cherry) instead of hijacking the cherry pipeline — HasPerk/SetPerk/HUD all work.
...
- **For our map:** Replace our _acc_perk_aura_blast cherry hijack with this chain in our own module — it is pure stock API (already verified style in docs/19) and removes the dependency on the unfinished stock cherry file.
...
- **For our map:** Our Phase-4 .csc work: Aura Blast needs exactly this mirror module; the tesla shock-eyes handlers are a drop-in template for Aura Blast's zap visuals.
...
- **For our map:** Direct answer to 'did anyone finish stock cherry': yes — use this as the verified reference if we ever ship a real Electric Cherry, and lift its stun/notify/anti-spam patterns into Aura Blast.
...
- **For our map:** Huge for our per-run randomization pillar: spawn the 9-perk roster at randomized struct locations from GSC each run instead of fixed hand-authored machines in the .map.
...
- **For our map:** Our greybox start-room sells perks before any power quest exists — use force_power so all 9 machines run from blackscreen; later swap to the stock <perk>_on/_off notifies when we add a power switch.
...
- **For our map:** Use level.perk_damage_override for any Cyberware/Overclock damage-reduction node and for Aura Blast self-damage immunity — no monolithic damage-function replacement needed.
...
- **For our map:** This is the full checklist our docs/13_perks.md needs for finishing Aura Blast with a real machine model: model x2 + bottle view/world models + weapon file + fx + shader image + szc alias block.
...
- **For our map:** Best end-to-end reference for inventing original perks (our Aura Blast is one): shows engine-specialty piggybacking, hold-to-use progress bars, cooldown flags, and teleport-safety checks reusable for any movement Cyberware.
...
- **For our map:** Aura Blast's placeholder model: legitimate shipped practice is reusing/re-skinning any stock vending xmodel and defining off/on pair — we can pick a cyber-looking stock machine (e.g. p7_zm_vending_*) now and swap later without touching script structure.
...
- **For our map:** Exactly the mechanism for our Overclock visual modifiers (per-run fog/lighting/eye-color mutations) once we write Phase-4 .csc modules.
...
- **For our map:** The exact Phase-4 recipe for our Data Shards counter, Cyberware tree state, and Overclock indicators — register clientuimodel ints/floats server-side, override t7hud_zm_custom.lua, one rawfile zone line per Lua file.
...
- **For our map:** Gives Aura Blast (and the Mega-upgrade 'paused/overclocked' visual state) a real icon; status=2 is a free 'perk disabled by Overclock' indicator.
...
- **For our map:** We currently rely on the stock zm_levelcommon_weapons.csv; this is the full verified path to custom costs/box pools for our roster, including the client-side trap.
...
- **For our map:** Our wallbuy roster expansion recipe: per added gun we need exactly one zone weapon line per asset variant + one CSV row; copy the dual-wield/_zm naming asymmetry into docs/19 to avoid the classic 'asset not found' first-compile error.
...
- **For our map:** Completes our docs/18 publish checklist: exact field names, the three image filenames, and the <map>_load.mkv loading-video convention.
...
- **For our map:** Extends our docs/18 publish checklist: after sound build, CachedBanks output must land in zone/snd/{all,en}/ or workshop audio is silent — countryside is the proof of the final folder shape.
...
- **For our map:** Our Phase-4 client modules should standardize on util::setClientSysState("levelNotify", ...) / clientnotify for power, Overclock activations, and boss-phase visual cues.
...
- **For our map:** Needed whenever our boss fight, traps, or Aura Blast kill zombies by script without skewing round progression.
...
- **For our map:** Our first-compile checklist should assert: scriptgdb/*.gsc.gdb exists for all 21 of our scripts, errorlog has no ^1ERROR, and use .deps to verify the linker consumed our subfolder modules — copy this into docs/18.
...
### MattFiler/zm_alien_isolation (GitHub, shipped Steam Workshop map Dec 2016; local clone at C:\Users\Jordan Urbaez\Repositories\abandoned_cyber_city_zombies\tmp\zm_ai_full)
...
Mod-tools overlay repo (366 files) for a shipped two-map project: a dev greybox (zm_building) and the full Abbey map, switched at runtime by GetDvarString("ui_mapname"). No .map sources committed — only scripts/, ui/ (Lua HUD), gamedata/ (weapon+BGB CSVs), sound/zoneconfig, 13-language localizedstrings, and zone_source/ (708-line .zone + modular .zpkg includes). It is the best available reference for: huge ported-weapon rosters (44-row zm_levelcommon_weapons.csv override + 88 zone weapon lines), a dvar-driven console-command framework (Sphynx), generator-gated power/box systems, challenge-quest perk upgrades, GSC-driven LUI menus, and an itemized list of every unfinished spot in stock _zm_perk_electric_cherry — which directly de-risks our Aura Blast cherry hijack.
...
- ohm-nabar/zm_building: scripts/zm/_zm_perk_electric_cherry.gsc (diff vs stock = checklist for our Aura Blast hijack)
...
Surveyed open-source BO3 custom-perk implementations across seven repos. The headline find is ColDog5044/zm_countryside — a complete shipped Workshop usermap that bundles the HarryBo21 Perks v3.1.0 framework in full source (utility framework + PHD Flopper, Electric Cherry, Tombstone, Who's Who, Vulture Aid, Widow's Wine, Elemental Pop, wonderfizz, plus the Lua HUD icon factory and the real .zone manifest). Second find: treminaor/ugx-mod-bo3 contains `_zm_perk_electric_cherry_fixed.gsc/.csc` — a finished, shipped Electric Cherry completion for BO3 (exactly what our Aura Blast hijacked the unfinished stock module for). Together these provide verified known-good patterns for every layer of a custom perk: GSC registration, CSC clientfield/FX halves, machine entities/KVPs, zone lines, sound aliases, and HUD icons.
