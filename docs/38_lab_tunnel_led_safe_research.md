...
- **`lightingstate1..4` = WHICH lighting states the light is lit in (power-gating!).** The block above with all four `"1"` = lit in EVERY state = NOT power-gated. To make lights "off until power", split by state and remember the **OFF-BY-ONE** (verified 2026-06-19 vs stock `zm_giant_light.map` + `util_shared.gsc`, also docs/22): GSC `set_lighting_state(N)` shows Radiant State **N+1** = `lightingstate(N+1)`; there is **no `lightingstate0`** key. This map's `CheckForPower` runs `set_lighting_state(0)` pre-power → `(1)` on power, so **pre-power = `lightingstate1`, post-power = `lightingstate2`**. Recipe used in `gen_room_roofs.js`: a DIM set (`lightingstate1="1"`, `2/3/4="0"`, low `bake_intensity_scale`) for the pre-power floor + a BRIGHT set (`lightingstate1="0"`, `2/3/4="1"`) for the powered look. Two lights per spot, one per state.
