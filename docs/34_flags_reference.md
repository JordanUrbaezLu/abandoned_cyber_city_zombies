...
| `acc_boss_mini_hp` | **`56000`** | Brutus (Trench Warden) **base** solo HP — SHARED with the Phantom/Rogue (unified boss scale, user 2026-07-04). The round-scaled value is then × the logarithmic co-op multiplier. Read at each Brutus spawn. | [`_acc_boss.gsc` `scale_mini_boss_hp`](../scripts/zm/zm_abandoned_cyber_city/_acc_boss.gsc) |
| `acc_boss_mini_hp_exp` | **`1.12`** | Brutus HP **compounding exponent per round** past the anchor — base × 1.12^(round−anchor) (user 2026-07-04, the TANKIEST tier: Brutus 1.12 > Rogue 1.1 > Phantom 1.08). Solo r10 56k → r20 174k → r30 541k → r40 1.68M. **No cap.** Read as a **float**; `1.0` = flat (no round scaling). | [`_acc_boss.gsc` `scale_mini_boss_hp`](../scripts/zm/zm_abandoned_cyber_city/_acc_boss.gsc) |
| `acc_boss_mini_hp_anchor` | **`10`** | The round at which Brutus HP equals the base (shared anchor with Phantom/Rogue); scaling only applies **past** it. He debuts at round ≥ 5, so rounds 5–9 sit at the flat base, then compound from r10. | [`_acc_boss.gsc` `scale_mini_boss_hp`](../scripts/zm/zm_abandoned_cyber_city/_acc_boss.gsc) |
...
| `acc_reactor_shielded_per_wave` | **`2`** | Reactor Surge: **Shielded ("Riot") elites** erupted from the pit risers **per wave** (user 2026-06-24). Each is 4× HP + front armor; flagged **no-shard** (gives no Data Shards on kill — a threat, not a farm, same as the glitch purge). `0` = none. | [`_acc_reactor.gsc` `reactor_spawn_specials`](../scripts/zm/zm_abandoned_cyber_city/_acc_reactor.gsc) |
...
| `acc_phantom_hp_exp` | `1.08` | Phantom HP **compounding exponent per round** past the anchor — base × 1.08^(round−anchor) (user 2026-07-04: 1.1 → 1.08, the SOFTEST boss tier — Brutus 1.12 > Rogue 1.1 > Phantom 1.08, all on the same 56k/anchor-10 scale). Solo r10 56k → r20 121k → r30 261k → r40 564k. Read as a **float**; `1.0` = flat. |
| `acc_protector_hp_exp` | `1.1` | Rogue Protector HP **compounding exponent per round** — the MIDDLE tier on the shared `scale_phantom_hp` scale (user 2026-07-04). Solo r10 56k → r20 145k → r30 377k → r40 977k. Read as a **float**. | [`_acc_civil_protector.gsc`](../scripts/zm/zm_abandoned_cyber_city/_acc_civil_protector.gsc) |
...
| `acc_phantom_slow_mult` | `0.75` | Move-speed multiplier applied by the chain-special **zap** when it connects — **−25%** (user 2026-06-26; was `0.70`/−30%). Read by `acc_utility::recompute_move_speed` while `acc_phantom_slowed`. |
| `acc_protector_slow_mult` | `0.75` | Move-speed multiplier for the **Rogue Protector** zap stun — **−25%**, matches the Phantom (user 2026-07-03, was `0.5`). Read while `acc_protector_slowed`. |
| `acc_boss_slow_mega_mult` | `0.90` | **Mega Electric Cherry "Power Surge"** softened boss-stun multiplier — **−10%** instead of full immunity (user 2026-07-03). Used in place of the `−25%` above when the slowed player holds live Mega EC (`acc_*_slow_mega` flag set in `_acc_elites::acc_phantom_chain_zap`/`acc_protector_zap`). |
