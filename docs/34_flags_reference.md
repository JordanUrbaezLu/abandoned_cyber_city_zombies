...
| `acc_trench_zombie_points` | `20` | Trench: **flat kill payout** for a surge/drip-spawned trench zombie (tagged `acc_trench_zombie`). These are a THREAT, not a payout — no damage-share split, headshot/knife bonus, or Kinetic Battery accrual, and they're excluded from the round count. Set `0` for no payout. (user 2026-06-24: 10 → 20.) | [`_acc_points.gsc` `on_zombie_death`](../scripts/zm/zm_abandoned_cyber_city/_acc_points.gsc) |
...
| `acc_boss_mini_hp` | **`40000`** | Brutus (Trench Warden) **base** solo HP at his **debut round** (`acc_boss_mini_hp_anchor`). The round-scaled value is then × the logarithmic co-op multiplier. Read at each Brutus spawn. | [`_acc_boss.gsc` `scale_mini_boss_hp`](../scripts/zm/zm_abandoned_cyber_city/_acc_boss.gsc) |
| `acc_boss_mini_hp_round_pct` | **`0.06`** | How much Brutus HP grows **per round** past the anchor — **simple (non-compounding)** +6% of base/round (user 2026-06-24 "scale with rounds but not crazy"): solo r5 40k → r10 52k → r20 76k → r30 100k → r40 124k. Read as a **float**. `0` = flat (no round scaling). | [`_acc_boss.gsc` `scale_mini_boss_hp`](../scripts/zm/zm_abandoned_cyber_city/_acc_boss.gsc) |
...
| `acc_launcher_boss_mult` | **`0.50`** | Mahem launcher damage **vs bosses/mini-bosses only** (boss-nuke audit, user 2026-06-24). Direct + splash both hit one boss hitbox; gentler cut than the Thundergun because the launcher is ammo-limited. On top of its 0.315 balance mult → ~2,756/rocket + splash vs a boss. `1.0` = no boss cut. | [`_acc_damage.gsc` `boss_nuke_mult`](../scripts/zm/zm_abandoned_cyber_city/_acc_damage.gsc) |
...
| `acc_reactor_reward` | **`5`** | Reactor Surge: Data Shards granted to **every** player on a successful survive (user 2026-06-24: `3` → `5`; also drops a shared Insta-Kill). | [`_acc_reactor.gsc` `reactor_reward`](../scripts/zm/zm_abandoned_cyber_city/_acc_reactor.gsc) |
...
| `acc_phantom_first_round` | `8` (base) / `4` (dev) | First round it can spawn (real cadence). Default is `4` when `acc_dev` is on. |
| `acc_phantom_interval` | `8` (base) / `4` (dev) | Then every N rounds — base game r8/16/24…, **dev mode every 4** (r4/8/12…) for testing. Either default is dvar-overridable. |
| `acc_phantom_test` | `0` | Dev/test path: spawn it every round from `acc_phantom_test_round`. **Also fires whenever `acc_dev` is on** (the default) — use `acc_dev 0` to see the true round-8 cadence. |
...
> **Final values (2026-06-24):** first spawn **round 10**, then **every round** (interval 1); **HP** flat
> **100k × player count**; **speed** `acc_phantom_speed_mult` **1.685** = fixed **+2% over a player's base
> sprint** (sprint gait, every round; tuned by in-game probe vs measured 299 u/s player sprint); **glow RED**
> (`acc/light/fx_perk_glow_red`). The `acc_phantom_first_round` / `acc_phantom_interval` / `acc_phantom_hp_mult`
> rows in the table above are stale vs these.
