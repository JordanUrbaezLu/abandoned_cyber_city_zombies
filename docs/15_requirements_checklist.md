...
- [x] **headshot-mult** `implemented` — Head-hit damage gets a +2.0x bonus on regular zombies/elites and +2.0x on bosses/mini-bosses (lowered from 3.0x, 2026-06-14) on top of stock GDT, with body/limb hits untouched, via the actor damage callback. Bonus stacks are ADDITIVE (summed), reductions multiplicative.
...
- [x] **deadshot-1.5x-per-shooter** `implemented` *(closed 2026-06-12; values updated 2026-06-14: attacker HasPerk(specialty_deadshot) -> +1.4 (+1.8 American Sniper Mega) in _acc_damage::on_ai_damage)* — Deadshot perk adds a 1.4x headshot damage bonus applied per-player (only the shooter's damage), ADDED into the headshot bonus sum alongside the 2.0x map bonus (additive stacking; key name is historical).
  - *Next:* In _acc_damage.gsc::on_ai_damage, add 1.4 (or 1.8 Mega) into bonus_sum on headshots when attacker HasPerk specialty for Deadshot (per docs/10_perks.md)
...
- [ ] **mega-american-sniper** `stubbed` — American Sniper (Deadshot Mega): headshot bonus +1.8 REPLACES base +1.4 (no double-dip, added into the bonus sum) and weapons have near-zero recoil (−70%); ADS snap still excludes bosses.
  - *Next:* In the Deadshot damage branch add 1.8 into bonus_sum when has_mega_perk (replace-not-stack) and add a no-recoil player flag; depends on deadshot-x15-headshot and specialty-string-consistency.
