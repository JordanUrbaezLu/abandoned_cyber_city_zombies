# 15 - Requirements Checklist (master tracker)

Originally generated from a 39-agent audit of every design doc vs the code +
map source (2026-06-12, ultracode pass), when the map was a **pre-build
greybox** and almost nothing was wired end-to-end. Statuses below: implemented =
works end-to-end now / scaffolded = structure + partial logic / stubbed = data
exists, effects log-only / map-missing = code ready, Radiant entities absent /
missing = no code / design-only = doc only / phase4-blocked = needs assets.

> **⚠ HISTORICAL SNAPSHOT — do not trust the per-item statuses or the totals
> table below as current.** They are frozen at the 2026-06-12 pre-build audit.
> A month of heavy dev has landed since (see CHANGELOG.md), and the map is now
> **FULLY BUILT** (first clean compile 2026-06-12; ~48 active `_acc_*` modules
> in `_acc_main.gsc` init()). The big premises this tracker was built on are
> now FALSE — corrections below. **The living docs + CHANGELOG.md are the
> source of truth; this file is kept for its per-requirement design intent, not
> its status column.** Re-auditing all 471 rows is a standing TODO.
...
- **Bosses are real, not log-only stubs.** Live modules `_acc_boss_avogadro`,
  ...
  driven by the shared multi-boss roster (`level.acc_boss_roster_fn`) and
  per-module debt directors. Boss cadence is **mini-boss first at round 10, then
  a full BOSS round every 9 from round 9** (r9=1, r18=2, r27=3), types dealt from
  a no-duplicate shuffled deck — NOT the frozen "boss every 10 / Subroutine Core"
  design. Every "boss reward path is unreachable" premise is false.
- **`_acc_perks.gsc` exists** — it owns the per-player perk cap, the cost table,
  and the stock-perk retunes. The perk cap is now **per-player** via the stock
  hook `level.get_player_perk_purchase_limit` (`acc_perks::acc_perk_slot_limit`),
...
- **The HUD is shipped, not Phase-4-blocked.** The Aetherium LUI HUD has been the
  ...
  health bars, and the gun-badge chip row (shipped 2026-07-08). Round progress is
  a smooth bar (the radial ring was abandoned). Every `phase4-blocked` HUD row and
  the "bottle clientfield is server-side only with no client counterpart" claim
  (docs/10 header) is stale.
- **The map grew far past the 7-zone greybox.** Beyond Plaza/Market/Alley/Bus/
  Vault/Helipad/Lab it now has the vertical **Abyss Descent** (L2/L3/L5 soul-box
  layers → Paradise plaza), the **bus trench**, **Exo Suit** station, **Armory**
  upper room, **Reactor Surge**, **Glitch Altar**, **Jukebox** (replaced the
  EE-song teddy bears), and **The Exchange** transfer vault — none tracked below.
  The underground is the Abyss Descent; the old "Black Market" split-room design
  was never built.
...
- [x] **zones-small-connected** `implemented` *(closed 2026-06-12: 7-zone greybox geometry landed)* — Map is multiple small, connected, readable zones (density over scale) rather than a single space.
  - *Next:* Build the remaining zones per docs/02_layout.md in Radiant with info_volumes, spawner structs, and zone-adjacency wiring in zm_zonemgr.
...
- [ ] **coop-4p-2hr-stable** `design-only` — Success criterion: 4-player co-op stays stable over a 2-hour run without persistent desync.
  - *Next:* After first Windows compile, run a 4-player 2-hour soak test per docs/34_release_runbook.md and log desync findings.
...
- [x] **doc-crossref-stale** `design-only` — Doc cross-references resolved in the 2026-07-10 renumber; the progression doc is `03_progression_and_skills.md` and all doc-to-doc links were rebuilt.
  - *Next:* Edit docs/00_overview.md:47 to reference 03_progression_and_skills.md.
- [ ] **wonder-weapon-identity** `phase4-blocked` — Open question to resolve: wonder weapon identity (nanite swarm vs EMP railgun vs code-injection pistol), answered in docs/04_weapons.md.
  - *Next:* Author the wonder-weapon GDT + assets on the Windows Mod Tools box and update docs/00_overview.md:50 to point at the resolved identity in docs/04_weapons.md.
...
- [ ] **alley-shotgun-wallbuy** `stubbed` — Alley has a shotgun wallbuy.
...
- [x] **zone-graph-8-edges** `implemented` *(closed 2026-06-12: 8 corridors + 8 add_adjacent_zone calls (always_on greybox flags))* — The zone graph has exactly 8 bidirectional connections: Plaza-Market, Plaza-Alley, Market-Corp, Alley-Corp, Corp-Vault, Corp-Roof, Vault-Lab, Roof-Lab (Lab reachable via two approaches).
  - *Next:* Build the 8 inter-zone connections (door triggers + debris/brush geometry) in Radiant and register zone adjacency in the entry script per the docs/02_layout.md mermaid graph.
...
- [x] **zones-min-two-exits** `implemented` *(closed 2026-06-12: every zone has 2+ corridor connections per the 8-edge graph)* — Every zone has at least two exits (no dead-end panic traps except the designed Vault Overload risk).
  - *Next:* Enforce the two-exit rule during the Radiant blockout of each zone; audit the final zone graph against the docs/02_layout.md mermaid edges before first compile.
...
- [ ] **spawn-wallbuys-pistol-smg** `missing` — Plaza features 2x pistol wallbuy, an SMG wallbuy, and a starting pistol upgrade terminal.
  - *Next:* Reconcile docs/02_layout.md Plaza features with the docs/04_weapons.md v1.0 roster (SMG dropped), then add the agreed spawn wallbuy slots to roll_wallbuy_pool and place the structs + pistol upgrade terminal in the Plaza zone.
...
- [ ] **roof-helipad-modifier** `missing` — A Helipad-specific modifier exists that forces players to move on timers (compensating for poor elite pathing on the Roof).
  - *Next:* Add the helipad movement-timer modifier to _acc_modifiers.gsc per docs/06_replayability.md and gate it on Roof-zone occupancy.
- [ ] **footprint-der-eisendrache** `design-only` — Total playable footprint is comparable to Der Eisendrache (small and dense), not Tranzit.
  - *Next:* During the zone blockout, measure total walkable area against a Der Eisendrache reference and record the budget in docs/02_layout.md.

## docs/03_progression_and_skills.md — Progression and Skills (Data Shards economy, Cyberware tree, difficulty curve, solo/co-op scaling). Code spine: _acc_data_shards.gsc, _acc_cyberware.gsc, _acc_elites.gsc, the _acc_boss_*.gsc roster, _acc_events_hack.gsc, _acc_emergency_drop.gsc, _acc_overclocks.gsc, _acc_boss_items.gsc, _acc_early_round_pacing.gsc, _acc_perks.gsc.

> **HEADER STALE (2026-06-12).** The frozen "headline gaps" — interaction
> entities absent from the .map, and boss/mini-boss actors are log-only stubs so
> every boss-derived reward path is unreachable — are FALSE now: the kiosk,
> terminals, power triggers, and `acc_boss_spawn` struct are in the .map, and the
> multi-boss roster is live (see the current-state corrections at the top of this
> file). Co-op spawn-count scaling reverted to the base game (2026-06-24). Rows
> below still capture the intended Cyberware/economy design; treat boss-cadence
> and reward-reachability wording as superseded.
...
- [x] **sr1-shard-regen** `implemented` — Subroutine Tier 1: +1 Data Shard per 2 minutes passive regen.
  - *Next:* Fix doc-internal inconsistency: flowchart (docs/03 line 55) says '+1 Shard/min' while body (line 79) and code say per 2 minutes.
...
- [x] **sr3-recursion** `implemented` — Recursion (SR T3): every 5th elite kill drops a random pickup from {max ammo, full armor, insta-kill, double points}.
  - *Next:* Reconcile the pool: either implement the armor pickup (depends on the armor system) or update docs/03 line 83 to list nuke.
...
- [ ] **sink-weapon-tier-15** `map-missing` — Sink pricing: Weapon Tier progression T1-T5 on one weapon totals 15 Shards (1/2/3/4/5 per step).
  - *Next:* Place trigger_use 'acc_overclock_terminal' in Radiant (effects implementation tracked under docs/04 items).
...
- [x] **tac19-no-headshot-mult** `implemented` — Tac-19 takes no headshot multiplier — damage is flat across hit location.
  - *Next:* Fix the doc's stale function reference: docs/04_weapons.md line 79 cites is_applicable_weapon(); the actual function is is_weapon_headshot_excluded().
...
- [ ] **wallbuy-intervention-3500** `scaffolded` — Intervention (MW2 import) is a 3,500-point wallbuy at Helipad; documented fallback is Drakon at the wallbuy if the port is unstable.
  - *Next:* Import intervention_zm and restore the doc-primary layout (Intervention wallbuy / Drakon box-strong), or amend docs/04_weapons.md to canonize the swap.
...
- [ ] **oc-utility-tiers-advance** `scaffolded` — Pistol, Melee, and Grenade weapons have no Overclock pool but tiers still advance (slot/stat purposes), just with no Overclock roll.
  - *Next:* Either let "none"-family weapons advance tiers without a roll, or amend docs/04_weapons.md line 209 to match the refusal behavior.
...
- [ ] **perk-costs-weapon-relevant** `scaffolded` — Weapon-relevant perk costs match: Deadshot 3,500; Speed Cola 3,500; Double Tap 2.0 2,000; Widow's Wine 4,000; PhD Flopper 2,500.
  - *Next:* Align Deadshot and Speed Cola costs (precache strings + actual perk-register costs) with docs/10_perks.md, or fix the doc values.
- [ ] **perks-9-roster-4-lockout** `scaffolded` — 9 perks available with 4 locked out per run (per docs/04 line 237; 13_perks/code use a per-ROUND 4-of-9 rotation locking out 5).
  - *Next:* Fix docs/04_weapons.md line 237 to say "4 available per round, 5 locked out, re-rolled per round" to match 13_perks and code.
...
- [ ] **wonder-craft-gates** `missing` — Crafting gates: Signal Staff requires Vault Overload completed this run + 5 Data Shards at the Lab terminal; Vibro Cleaver requires Hack Terminal completed this run + 5 Shards at the Vault terminal.
...
- [x] **perks-no-cap** `implemented` *(mechanism changed since 2026-06-12)* — The stock 4-perk cap is lifted so one player can hold all **10** perks. Implemented **per-player** via the stock hook `level.get_player_perk_purchase_limit = &acc_perks::acc_perk_slot_limit` (`_acc_perks.gsc:73`, `ACC_PERK_SLOT_MAX = 10`); the entry script sets only the base `level.perk_purchase_limit = 4` (`zm_abandoned_cyber_city.gsc:320`), which the per-player hook overrides. (The old "set `perk_purchase_limit = 9` in main()" note is superseded — 9 was never the count and the global is no longer the real cap.)
...
- [ ] **wallbuy-m14ebr-1500** `phase4-blocked` — M14 EBR (MW2 import) is a 1,500-point wallbuy at Bus Station slot B, separate from ICR-1.
  - *Next:* Author m14ebr_zm GDT + weaponfull line, then swap the map struct from ar_marksman to the import (or document Sheiva as the interim stand-in in 04_weapons.md).
...
- [x] **points-stock-suppression** `implemented` — Stock kill awards (60/100/130) are fully suppressed while per-hit 10-point damage awards are kept unchanged.
  - *Next:* Update docs/05_mechanics.md 'Stock-Award Override Status' section to reflect the wired register_score_event suppression
...
- [x] **headshot-mult** `implemented` — Head-hit damage is tempered by `resolve_headshot_multiplier` (`ACC_HEADSHOT_MULT = 0.5` regular/elite, `ACC_BOSS_HEADSHOT_MULT = 0.8` boss/mini-boss, `_acc_damage.gsc:88-89`) applied MULTIPLICATIVELY on top of the engine's locHead (~5.0), netting ~2.5x on regular zombies/elites and ~4x on bosses/mini-bosses (user 2026-06-25 "2.5x reg / 3x boss"; boss raised to 4x on 2026-07-08). For a REAL headshot the additive base layer is skipped (`if(!b_headshot)`, `_acc_damage.gsc:726-730`) and the temper multiplies separately so locHead doesn't balloon the other bonuses; BODY crits (Precision Mode / Cyberware proc) keep the additive base. Body/limb hits are untouched, via the actor damage callback. Bonus stacks are ADDITIVE (summed), reductions multiplicative.
...
- [x] **hack-three-stages** `implemented` — Hack runs 3 back-to-back stages: (1) 10 kills in 40s, (2) 3 Shielded elite kills in 60s, (3) 15 headshot kills in 45s, counted via the zombie-death callback.
  - *Next:* Document the pre-r11 stage-2 fallback in docs/05_mechanics.md (or remove it if the doc's fixed stage list is intended)
...
- [x] **phd-flopper-recovery** `implemented` — PhD Flopper's dive-to-prone nova explosion clears nearby zombies (plus fall/self-explosive immunity), giving a repositioning/recovery option.
...
- [x] **deadshot-1.5x-per-shooter** `implemented` *(closed 2026-06-12; values retuned 2026-06-25: attacker HasPerk(specialty_deadshot) -> +1.3 (+1.5 American Sniper Mega) in _acc_damage::on_ai_damage, `ACC_DEADSHOT_MULT`/`ACC_DEADSHOT_MEGA_MULT` = 1.3/1.5 at `_acc_damage.gsc:105-106`)* — Deadshot perk adds a +1.3 headshot damage bonus applied per-player (only the shooter's damage), ADDED into the headshot bonus sum alongside the map headshot temper (additive stacking; key name is historical).
  - *Next:* In _acc_damage.gsc::on_ai_damage, add 1.3 (or 1.5 Mega) into bonus_sum on headshots when attacker HasPerk specialty for Deadshot (per docs/10_perks.md)
...
## docs/06_replayability.md audit. Key files examined: scripts/zm/zm_abandoned_cyber_city/_acc_map_randomizer.gsc (Tier 1 per-run state), _acc_modifiers.gsc (Tier 3), _acc_overclocks.gsc, _acc_events_hack.gsc, _acc_early_round_pacing.gsc, _acc_cyberware.gsc, _acc_data_shards.gsc, _acc_elites.gsc, _acc_main.gsc, the entry script, and map_source/zm/zm_abandoned_cyber_city.map.

> **HEADER STALE (2026-06-12).** Headline finding (1) — "the map contains ZERO
> `acc_*`-targetnamed entities, so every randomizer application path is
> map-missing" — is FALSE now: the .map carries the power-switch, PaP-blocker,
> `acc_lab_perk_*`, `acc_box_*`, `acc_overclock_terminal`, and `acc_hack_terminal`
> entities (see the top-of-file corrections). The other findings below still hold
> at the time of audit and are worth verifying: (2) most modifiers set
> `level.acc_mod_*` vars with few consumers; (3) the `_acc_overclocks.gsc` family
> tables used legacy `_zm`-suffixed names while stock weapons are class-named, so
> `weapon_name_to_family` returned "unknown" for stock guns
> <!-- TODO(acc-verify): confirm this classifier was fixed -->; (4) the Rooftop
> sniper wallbuy is Drakon in code/map with intervention in the box, vs the doc's
> Intervention-wallbuy; (5) score-multiplier and meta-progression had no code.
> Wiring chain is sound: the entry script calls `acc_main::pre_init` →
> `acc_modifiers`/`acc_map_randomizer` pre_init, `early_round_pacing::post_zm_main`,
> `acc_main::init`; both modules are in the zone manifest.
...
- [ ] **wallbuy-slot-table-6-slots** `scaffolded` — Six wallbuy slots with exact weapons: Alley shotgun=Haymaker 12, Corp full-auto AR=ICR-1, Corp semi-auto AR=M14 EBR, Rooftop sniper=Intervention, Vault tactical=EMP Grenade, near-perk melee=Bowie Knife.
  - *Next:* Reconcile the sniper slot: either update docs/06:40 + docs/04 to Drakon-wallbuy/Intervention-box (matching code+map) or swap code/map once the intervention_zm GDT import lands; m14ebr_zm and emp_grenade_zm need Phase 4 GDT/weapon-table entries.
...
- [ ] **box-bad-strong-pool** `scaffolded` — Mystery Box pool delivers the bad/strong 50/50 tension per family (one bad + one strong per family registered as box-only).
  - *Next:* Author Phase 4 weapon GDTs + weapon-table CSV rows for the 5 import weapons so the registered pool matches docs/04.
- [ ] **cyberware-tree-3x3** `scaffolded` — Cyberware build space: 3 branches x 3 tiers with per-branch-tier mutual exclusion, producing the doc's ~27 end-state combinations.
  - *Next:* Reconcile the tier-2 choice count (code has 2 per branch vs doc's 3) — update docs/06:102 or add the third tier-2 node per branch per docs/03.
...
- [x] **perk-rotation-post-decon-timing** `implemented` *(closed 2026-06-12 second ultracode pass)* — Perk machines re-roll AFTER each round's decontamination phase completes, not at the first frame of the round.
  - *Next:* Author _acc_decontamination.gsc (20s evac + seal per docs/02) that notifies 'acc_decontamination_complete' with round_number, then swap the waittill in watch_round_for_perk_rotation.
...
- [ ] **elite-shards-to-killer** `scaffolded` — Elite shard drops are credited to the killing player (co-op rule), while still dropping on the body per design rule 5.
  - *Next:* Decide the policy: either tag the pickup with the killer (claimable only by killer for the first N seconds, then anyone) or amend docs/08_enemies.md to say drops are free-for-all body pickups.
...
## docs/09_boss_items.md — Boss Items (Machin[a]-style randomized passive-buff drops). 6-item pool, 2 equipped slots per player, drops from mini-boss (50%) and full boss (100%), duplicates convert to 3 Data Shards, no cross-run persistence. Primary module: scripts/zm/zm_abandoned_cyber_city/_acc_boss_items.gsc (pool + drop/equip/pickup plumbing real; some item effects were flag-only at audit time). Ledger bonus lives in _acc_points.gsc.

> **HEADER STALE (2026-06-12).** "Drop triggers live in _acc_boss.gsc, whose boss
...
## docs/10_perks.md. Audit summary: PhD Flopper hijacks the stock _zm_perk_electric_cherry pipeline via _acc_perk_phd_flopper.gsc (real cost/hint + machine-identity fix; custom dive-explosion + fall/self-explosive immunity ability). The perk machines and wallbuy set exist in the map. The Mega Bottle economy (grant/consume/flag/persistence API) is real code in _acc_mega_bottles.gsc.

> **HEADER STALE (2026-06-12).** Several premises here are FALSE now: (a)
> `_acc_perks.gsc` **exists** and owns the perk-cap removal, cost table, and
> stock-perk retunes — it is NOT "planned-but-absent"; (b) the perk cap is
> per-player via `level.get_player_perk_purchase_limit`
> (`acc_perks::acc_perk_slot_limit`, `ACC_PERK_SLOT_MAX = 10`) — this is a
> **10-perk** map (Electric Cherry is the real 10th), so the rotation is a
> **4-of-10** roll, not 4-of-9; (c) boss spawns are real, so the Mega Bottle
> economy has live producers; (d) the HUD is NOT Phase-4-blocked — the Aetherium
> HUD (`_zm_aetherium_hud.gsc/.csc`) registers the bottle clientfield **on both
> the .gsc AND .csc side** (`acc_mb` toplayer field) and shows the bottle counter.
> The specialty-string / rotation-timing observations may still be worth checking.
> <!-- TODO(acc-verify): re-audit Mega effect rows + specialty-string consistency against _acc_perks.gsc + _acc_mega_bottles.gsc -->
> See the top-of-file corrections.
...
- [ ] **mega-ultimate-tank** `stubbed` — Ultimate Tank (Jug Mega): 7 melee hits from full to down (+1 over this map's 6) and immunity to boss scripted disables (power-off, perks-off, boss stun fields).
  - *Next:* Implement the 7-hit HP tier and add per-player Ultimate Tank exemptions inside _acc_boss.gsc phase debuffs (whitelist per docs/10_perks.md line 70 TODO(acc-impl)).
...
- [x] **mega-gun-slinger** `built` *(reworked 2026-07-04)* — Gun Slinger (DT2 Mega): a runtime damage buff ONLY — the base Double Tap extra-bullet temper eases **×0.6 → ×0.8** for Mega holders (dvar `acc_doubletap_mega_dmg_mult` = 0.8 in `_acc_damage.gsc` via `has_mega_perk`), giving Mega ~2.1× DPS vs base ~1.6×. The `fastfire` weapon-variant twin (+45% fire rate + −50% weapon-swap) was **REMOVED ENTIRELY 2026-07-04** — there is no twin, and weapon-swap is no longer a perk effect. UI card `acc_hud.lua` now reads "Extra bullets hit harder".
...
- [ ] **mega-american-sniper** `stubbed` — American Sniper (Deadshot Mega): headshot bonus +1.5 REPLACES base +1.3 (`ACC_DEADSHOT_MEGA_MULT`/`ACC_DEADSHOT_MULT` at `_acc_damage.gsc:105-106`; no double-dip, added into the bonus sum) and weapons have near-zero recoil (−70%); ADS snap still excludes bosses.
  - *Next:* In the Deadshot damage branch add 1.5 into bonus_sum when has_mega_perk (replace-not-stack) and add a no-recoil player flag; depends on deadshot-x15-headshot and specialty-string-consistency.
...
- [x] **perks-no-cap** `implemented` — The stock 4-perk cap is removed so one player can hold all **10** perks. Cap is per-player via `level.get_player_perk_purchase_limit = &acc_perks::acc_perk_slot_limit` (`_acc_perks.gsc:73`, `ACC_PERK_SLOT_MAX = 10`); the entry script sets the base `level.perk_purchase_limit = 4` (`zm_abandoned_cyber_city.gsc:320`).
- [x] **acc-perks-module** `implemented` — `scripts/zm/zm_abandoned_cyber_city/_acc_perks.gsc` **exists** (live in `_acc_main.gsc:49` #using), owning the per-player perk cap (`acc_perk_slot_limit`), the custom perk registration/retunes, and the cost table. It is `#using`d and in the zone manifest. <!-- TODO(acc-verify): confirm the full effect list (Deadshot/Widow hooks, Jug/QR/Speed/Stamin retunes) all landed in _acc_perks.gsc -->
  - *Next:* Cross-check the individual retune rows below against `_acc_perks.gsc` before flipping each from `missing`/`scaffolded`.
...
- [ ] **speed-cola-qol** `missing` — Base Speed Cola adds ~40% shorter perk-drink animation and ~30% faster weapon+equipment swap on top of stock +50% reload.
  - *Next:* Implement drink-duration and weapon-swap-speed modifiers for specialty_fastreload owners in _acc_perks.gsc (resolve the TODO(acc-tune) stacking rule from docs/10_perks.md line 88).
- [ ] **dt2-plus3-damage** `missing` — Base Double Tap 2.0 on this map = stock +33% fire rate plus a +3% flat weapon damage line applied in the single _acc_damage pipeline (design abstraction replacing stock double-bullet).
  - *Next:* **Superseded 2026-07-04** — the shipped design is the extra-bullet damage temper (base ×0.6, Mega Gun Slinger ×0.8 via dvar `acc_doubletap_mega_dmg_mult` = 0.8 in `_acc_damage.gsc`), NOT a separate +3%/+6% damage multiplier. No further work; see mega-gun-slinger above.
- [ ] **deadshot-x15-headshot** `missing` — Deadshot base adds **+1.3** headshot bonus into the crit pool (American Sniper Mega replaces with **+1.5**); the map headshot temper (`locHead × 0.5` trash / `× 0.8` boss → ~×2.5/×4 on a locHead-5.0 gun) multiplies on top, the Deadshot bonus adds into the pool. Additive into the pool, not a separate multiplier (2026-06-14).
  - *Next:* Extend _acc_damage::on_ai_damage to add 1.3 (or 1.5 when acc_mega_bottles::has_mega_perk for Deadshot) into bonus_sum when attacker HasPerk('specialty_deadshot') and the hit is a headshot.
- [ ] **widow-grenade-boosts** `missing` — Base Widow's Wine adds +50% frag damage / +25% frag radius and +50% EMP stun / +25% EMP radius on top of stock web behavior.
  - *Next:* Implement grenade damage/radius boosts for specialty_widowswine owners (hook grenade fire / actor damage MOD_GRENADE paths) in _acc_perks.gsc as listed in docs/10_perks.md Implementation Status.
...
- [ ] **no-perk-bottle-powerup** `missing` — The Perk Bottle / random-perk-drop powerup is NOT in v1.0.
  - *Next:* Either remove _zm_powerup_free_perk from both entry scripts and replace the emergency-drop 'random_perk' reward, or amend docs/10_perks.md line 342 to document the emergency-drop exception.
...
- [x] **weapon-ability-input** `implemented` — Weapon Ability has a working Phase-3 activation input (doc claims a console `bind h notify acc_ability` listener).
  - *Next:* Update docs/11_controls_and_hud.md Custom bindings row: activation is the ADS+melee chord, not `bind h notify acc_ability` (doc is stale vs code).
...
## docs/12_coop_rules.md audit. Headline findings (2026-06-12 audit):

> **HEADER PARTLY STALE (2026-06-12).** Finding (5) — "every custom co-op
> interactable is absent from the .map; grep `acc_` returns zero matches" — is
> FALSE now (see the top-of-file corrections). Finding (4)'s Vault Overload half
> is moot: that side-event was RETIRED 2026-07-07. Finding (1) is now partly by
> design: co-op zombie **spawn-count** scaling was deliberately reverted to the
> **base game** (2026-06-24, the `base-game` rows below), so the absence of a
> custom spawn-rate rescale is intended; elite **HP** does scale per player. The
> remaining findings below still merit verification: (2) down/revive deltas and
> bleed-out multiplier consumption; (3) respawn "keep all weapons";
> (4) Hack stage counts activator-only kills; (6) Mule Kick cost vs the doc's
> 2500 <!-- TODO(acc-verify): confirm current Mule Kick cost -->; (7) the
> doc-internal 4-items-vs-1-item contradiction (code drops exactly 1).
...
- [x] **kill-points-70-30-split** `implemented` — Regular kill = 40 pts (killer 70% / contributors split 30%), headshot/knife = 100 pts same split, solo kill = 100% to killer.
  - *Next:* Update docs/12_coop_rules.md:110-112 (and docs/05) split examples to the 10-point-quantized payouts (30/10 on a 40-pt kill).
...
- [x] **mule-kick-per-player-repurchase** `implemented` — Mule Kick is bought per-player; losing it (death/respawn) removes the 3rd weapon, recoverable by repurchasing the perk.
  - *Next:* Amend docs/12_coop_rules.md:166 wording ("reverts to the floor") to match stock store-and-return behavior.
...
- [x] **overload-tether-8s-cumulative** `implemented` *(closed 2026-06-12 second ultracode pass)* — Vault Overload: only the activator is tethered; leaving the point >8s CUMULATIVE fails the event (other players roam freely).
  - *Next:* Remove the reset at _acc_events_overload.gsc:158 to make the 8s truly cumulative, or amend docs/12_coop_rules.md:152 to the lenient rule.
...
- [ ] **easymode-4-items-claim** `design-only` — "4-Player Easy Mode" section claims 4 boss items per boss kill (up to 2 per player, extras convert to Shards) — contradicting the 1-item-per-boss rule earlier in the same doc.
  - *Next:* Reconcile docs/12_coop_rules.md:174-175 with the Boss Item Drops section (decide: 1 shared drop, or 1 per player), then update _acc_boss_items.gsc only if the per-player variant wins.
