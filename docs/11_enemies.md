...
> **Implemented as Brutus** (NSZ pack), not the old "Juggernaut Host" placeholder below. **Spawn cadence (user 2026-06-18): FIRST appears when the Bus Station POWER is turned on** (`_acc_boss::brutus_power_watch` — was a fixed round 4), then **every 5 rounds** from that anchor. Runs alongside the wave (`ignore_enemy_count`), drops a boss item + Mega Bottle. **HP (user 2026-06-24): scales with the round** like a regular zombie does, but TAMER than stock's compounding curve so a high-round Warden is never "crazy" — base **40k at his debut round (5)**, then a **simple (non-compounding) +6% of base per round past it** (`acc_boss_mini_hp` / `acc_boss_mini_hp_round_pct` / `acc_boss_mini_hp_anchor`), e.g. solo **r5 40k → r10 52k → r20 76k → r30 100k → r40 124k**. That round-scaled base is then × a **logarithmic co-op multiplier** (`boss_hp_player_mult`: ×1 / 1.5 / 1.8 / 2.0 for 1–4 players). PLANNED: tether it to **roam the Bus Station (corp_zone) trench** as a true "warden." The stale "Juggernaut Host / rounds 10,20 / 500k HP" details below are superseded.
