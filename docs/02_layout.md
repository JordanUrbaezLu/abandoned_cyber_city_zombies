...
- **Mystery Box location**: standard BO3-style box move among the six chests; the **initial** spawn is deterministic — `roll_mystery_box_initial` always returns `plaza` (the start room), and the stock teddy-bear move rotates among all six chests thereafter.
