...
- **Plaza→Alley connector** (x[213,1175] y[400,656], dressed 2026-08-03): a 10-prop **density gradient** — classical plaza remnants at the west mouth (S-wall bollard cordon + dead stone planter + weeds) fading through litter and a blue LED strip into the Alley's red-hazard language (wall electric set, a wire-bundle run that visibly feeds the rampage breaker at (1020,412), biohazard sign at the door; a red cage beacon was cut - a 2nd baked instance of that light model crashes the LED bake). All floor solids hug the S wall; the N side stays the walk/train lane (≥189u). Clips: `add_prop_clips.js` `plaza_conn_*`.
...
- **COMPRESSED 2026-08-02** (`tools/oneshots/gen_compress_lab_paradise.js`; user: "the lab is a place players push towards to be comfortable - make it smaller with more lab models"): playable interior is now **x[-761,799] y[3068,3868]** (1560x800, ~31% less area). A new inner N wall (hexagon, y[3868,3888]) seals the dead band left by the 2026-07-25 alcove-row removal; the outer envelope (walls/floor/ceiling to y4248) is untouched, plaza-shrink style, so rooms.json stays valid. The **Scientist's Office moved -360y with it** (doorway now cuts the inner wall at x[-123,-27]; the old outer-wall doorway is plugged). Everything in the sealed band relocated inward: perk staging structs y4195->3800, LED strips y4227->3867, riser row y3948->y3750..3808, roof-light row y4068->3800, neon pool + purple flare (0,4060)->(0,3830), and the E-wall industrial corner relayed onto the new N wall + an NE spur. **Densified with +18 already-zoned statics** (W-center data island: blue generator / computer tower / server comm / dragon terminal / holo; a 2nd specimen test chamber; standing console banks + wire sockets + a security-monitor pair on the new N wall; a center-south morgue/medical cluster; paper litter) - every floor prop clipped (`add_prop_clips.js` `lab_` prefix, anti-perch gables). The room now reads dense, not safe.
- **Features**:
  - **Pack-a-Punch machine** (L1-L5 progression via Points, see [04_weapons.md](04_weapons.md)).
  - **THE SCIENTIST'S OFFICE** (2026-07-26, `tools/oneshots/gen_scientist_office.js`) — a small buyable room behind the Lab (inner) N wall (interior `x[-275,125] y[3888,4208]` since the 2026-08-02 -360y translate; ceiling 256; hexagon-tile shell matching the Lab; the crown neon band continues over the door header). **Door: 2000 pts** (`enter_scientist_office`, diamond-plate slab in the doorway cut `x[-123,-27]`; standard dual radius buy-triggers). Lore: it's the **gun-stealing Scientist boss's office** (docs/44). Inside:
    - **Exo Suit station** (`_acc_exo`, spawn `(-200, 4120, 0)`) — Data-Shard Exo tiers, blue unused-station pulse.
    - **Juggernog — permanent** (perk-scatter `fixed_spec` pad at `(92, 4060)`, E wall; never rotates, solo or co-op — the office is always Jugg + Exo).
...
| **the Scientist's Office** (was Lab N wall) | (92, 4060, 0) | E wall (face x=125) **inside the office room** behind the Lab inner N wall (`gen_scientist_office.js`, translated -360y 2026-08-02), yaw 270 faces W | **PERMANENTLY Juggernog** (user 2026-07-26, "jugg and QR are always in same spot", **regardless of solo/co-op** — 2nd `fixed_spec` pad; `apply_scatter`'s fixed flow generalized off its QR hardwire the same day). Behind the 2000-pt `enter_scientist_office` door; clear of the chem-tank clip (y[3905,3959]) + control-console clip (y[4142,4202]); unstick push 85 lands (7,4060) open room center. Mega-pin refused on fixed perks (no bottle waste) |
