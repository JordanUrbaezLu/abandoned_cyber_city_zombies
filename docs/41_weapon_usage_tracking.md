...
- **Label-deploy caveat (2026-07-21):** the Worker labels `/stats/guns` from its own
  INLINED copy of the map (`worker.js` `GUN_IDS`, regenerated by the same generator), so a
  gun added to the registry is only labelled server-side after a Worker **redeploy**
  (`npm run deploy`) — until then `/stats/guns.json` returns the raw numeric id as the name
  (this is how the CYBERJACK/id 32 read back as `"32"`). `npm run summary` no longer depends
  on that: it resolves gun names LOCALLY from `tools/gun_ids.json` `_display` (preferring the
  registry over the Worker's label), so the summary is correct the moment the registry is,
  independent of the Worker's deploy cadence. Redeploy the Worker to also fix the live API.
