...
> - **Explosive specials:** Mahem `s1_mahem` (molten-metal rocket) + War Machine `t6_war_machine` (BO2 6-round drum GL, IMPACT detonation — GDT-native `fuseTime 0`; PaP "Dystopic Demolisher" = 12-rd full-auto drum; ~2420 direct/shot after two +10% buffs (2026-07-12, 2026-07-16), still just below the Mahem's 2500 per-shot — the drum burst is the appeal; Speed-Cola-only fastreload twins). **Added 2026-07-25:** **EPG-1** `s1_mdl` (the AW MDL grenade launcher **reskinned** into a neon plasma energy-**LOBBER** — slow high-arc **TIMED-fuse airburst** via `tools/oneshots/reskin_mdl_epg.js`: `projectileSpeed 3500→1400`, `projImpactExplode 0` on base + `_up`, explosion/trail/muzzle FX repainted to stock EMP/energy, base raw normalized **UP** to the `_up` so `bal` stays `<1` and the PaP tier ladder is the sole progression; **fire sound wired** — 4 aliases cloned from the Mahem, sound bank rebuilt). **+50% BUFF user 2026-07-26** (damage / clip / reserve, applied RAW-side in the GDT via the same oneshot so bal stays **0.68**): inner 900→**1350**, direct 600→**900**, outer 125→**188** → **~2984 direct base**; clip 6→**9**, base reserve 18→**27** rounds, PaP reserve 12 clip-relative mags → **108** rounds. An exempt explosive special (launcher boss-cut ×0.50 + explosive amp ×1.5 + the 10%/hit boss cap), TOP-band box odds (weight ~128), **dev-given in `acc_dev`** (`_acc_dev::dev_give_starting_guns`), and **Warhead Bomber implant (+20% explosive) applies** — verified in the `_acc_damage` gate (explosive MOD, not on the energy list). *(The **L4 Siege** `launcher_multi` — the stock zm_levelcommon rocket launcher added alongside it — was **REMOVED from the game 2026-07-26** by user request: CSV row, box pool/weight, PaP bucket/un-exempt, damage entries, and dev-give all deleted; the CSV-only inclusion recipe for stock-cooked guns lives on in memory `zm-levelcommon-stock-launchers-csv-only`.)*
...
Overall: **no perk cap** in this map (slot cap `ACC_PERK_SLOT_MAX` = 10). 10 perks total (the 9-specialty
roster in `get_full_perk_roster()` + Electric Cherry, wired separately); perk availability is the **map-wide
perk scatter** — machines live on 10 pads across the map and the perk→pad assignment reshuffles every 3rd
round (`_acc_perk_scatter.gsc`; replaced the Lab alcove-door rotation 2026-07-24). See
[10_perks.md](10_perks.md) for the authoritative roster, costs, and scatter rules.
