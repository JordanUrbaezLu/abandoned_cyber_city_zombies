...
- [x] **deadshot-1.5x-per-shooter** `implemented` *(closed 2026-06-12; values updated 2026-06-14: attacker HasPerk(specialty_deadshot) -> +1.4 (+1.8 American Sniper Mega) in _acc_damage::on_ai_damage)* — Deadshot perk adds a 1.4x headshot damage bonus applied per-player (only the shooter's damage), ADDED into the headshot bonus sum alongside the 2.0x map bonus (additive stacking; key name is historical).
  - *Next:* In _acc_damage.gsc::on_ai_damage, add 1.4 (or 1.8 Mega) into bonus_sum on headshots when attacker HasPerk specialty for Deadshot (per docs/13_perks.md)
...
- [ ] **acc-perks-module** `missing` — A Phase 3 _acc_perks.gsc module exists owning cap removal, custom perk registration, Deadshot/Widow effect hooks, stock-perk retunes, the cost override table, and the tuning-lever constants (Deadshot headshot bonus now +1.4, Widow +50->+25%, Stamin 2x->1.5x).
  - *Next:* Author _acc_perks.gsc per docs/13_perks.md Implementation Status list (cap removal, Deadshot mult feed, Widow grenade hooks, Jug/QR/Speed/Stamin retunes, cost table), #using it from _acc_main.gsc and add the zone line.
