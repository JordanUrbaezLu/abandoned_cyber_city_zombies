...
**Play** — double-click **`PLAY_NORMAL.bat`** (or `.\tools\run_game.ps1`). It
launches through Steam with the right engine args (engine args only — dev/god
are not launch flags).
...
You spawn in Spawn. **Dev/god are NOT launch flags** — for the dev sandbox
(unlimited money + Data Shards, all perk slots) hardcode `level.acc_dev = true;`
in `acc_resolve_dev_flags()` (`scripts/zm/zm_abandoned_cyber_city.gsc`) and
rebuild; bosses run their **real** cadence either way (the early test spawns were
removed 2026-07-16). Power and the buyable doors are **not** auto-granted even in
dev — you flip the Bus Station power switch and buy the doors yourself (trivial
with unlimited money). A build with the ship line `level.acc_dev = false;` (the
default) is **the clean consumer game** (earn your own money, decon hazard live).
Full list + recipes: [docs/22_flags_reference.md](docs/22_flags_reference.md)
(`-NoDev`, `-ClosedMap` and the other legacy `run_game.ps1` switches are all
no-ops kept only for call compatibility — clean vs dev is decided by the
hardcoded booleans in the build, not any launch switch). (reconciled to code 2026-07-16)
...
```
edit in the repo  →  .\tools\sync_to_modtools.ps1  →  build  →  PLAY_NORMAL.bat
```
...
> stay in lockstep** — regenerate and re-list all three together, then full-Compile. Swap
> prints show in a dev build (debug output rides `level.acc_dev`; the per-feature debug
> dvars were removed 2026-07-16).
...
| Balance/tuning dvars + the dev/god hardcode | [docs/22_flags_reference.md](docs/22_flags_reference.md) |
