...
### Changed — Mega Widow's Wine: removed the one-hit melee (user 2026-06-29)

Mega Widow's Wine ("Spiderman") no longer grants a **one-hit knife** on regular zombies — a Mega-Widow's melee now does **normal melee damage** (the OHK short-circuit block in [_acc_damage.gsc](scripts/zm/zm_abandoned_cyber_city/_acc_damage.gsc) `on_ai_damage` was deleted). Its remaining Mega effects are unchanged: **low-stance spider-mobility** (crouch ×2.6 / prone ×10 / downed ×15) and the **+10pp spider-drop** rate. (The OHK had been re-added 2026-06-18; the boss-special immunity moved to Mega Electric Cherry 2026-06-25.) GSC-only build.

### Added — Trench Brutus 3,000-point kill reward (user 2026-06-29)

The **Trench Warden (Brutus)** now also pays **every player 3,000 points** on death, on top of the existing 3 flat Data Shards + guaranteed boss item + 50%-chance Mega Bottle ([_acc_boss.gsc](scripts/zm/zm_abandoned_cyber_city/_acc_boss.gsc) `watch_mini_boss_death`, tunable `acc_warden_score_reward`). **The Paradise-fight Brutus pays nothing** — confirmed it already did (it spawns on a separate `spawn_one_paradise` path and runs a count-only `brutus_death_watch`, never threading the reward handler); the reward block is now *additionally* wrapped in a `!acc_no_shard_reward` guard (the Paradise Brutus is flagged `acc_no_shard_reward = true`) so the exclusion is bulletproof + self-documenting. GSC-only build.

### Added — Round-scaled Phantom kill reward (user 2026-06-29)

Killing a **Phantom mini-boss** now pays **every player** a round-scaled bounty: **round × 500 points + round × 1 Data Shards** (round 10 = 5,000 points + 10 shards; round 20 = 10,000 + 20; scales with the round it dies on). Replaces the old flat 5-shard payout in [_acc_boss_phantom.gsc](scripts/zm/zm_abandoned_cyber_city/_acc_boss_phantom.gsc) `phantom_death_watch`; tunable via `acc_phantom_score_per_round` (500) / `acc_phantom_shards_per_round` (1) — the old `acc_phantom_shard_reward` is retired. **Excluded from the Paradise fight**: the reward sits inside the existing `!acc_paradise_onslaught` gate (with the item drop + Mega Bottle), so the up-to-4 Phantoms cycling through the finale grant nothing (survive-don't-farm). GSC-only build.

### Changed — Global damage ×3.25 + Mahem −27.5% (user 2026-06-29)

The map's "buff all guns" lever `acc_global_dmg_mult` (`ACC_GLOBAL_DMG_MULT`, [_acc_damage.gsc:98](scripts/zm/zm_abandoned_cyber_city/_acc_damage.gsc#L98)) raised **2.75 → 3.25** (+225% on every gun, applied as the final flat multiply in `on_ai_damage`). To stop the already-strong **Mahem** launcher from scaling out of hand alongside it, its per-gun balance multiplier `acc_weapon_balance_mult("s1_mahem")` ([_acc_damage.gsc:898](scripts/zm/zm_abandoned_cyber_city/_acc_damage.gsc#L898)) is cut to **0.29 (0.40 → 0.29, −27.5%)** — `IsSubStr` matches **base `s1_mahem` + PaP `s1_mahem_up`**, scaling every route (direct + splash). Net for the Mahem: 3.25/2.75 × 0.29/0.40 ≈ **0.857**, i.e. **~14% below** its old output despite the +18% global bump. GSC-only build.

### Changed — Hip-fire spread re-balanced per weapon class (user 2026-06-29)

Hip-fire is now class-gated so every gun rewards ADS (snipers most of all). [tools/scale_hipspread_by_class.js](tools/scale_hipspread_by_class.js) is the **single source of truth** — it scales the 8 hip-spread **pattern** fields (Stand/Ducked/Prone/Slide Min+Max; the cone size — **ADS is NOT touched**) per gun across **base + PaP + all 14 twins + the `.acc-orig` recoil baselines**, always from a pristine `.acc-hipspread-orig` backup (idempotent + re-runnable; `--revert` resets every mapped gun to vanilla). Factors:

| Class | × | Guns |
|---|---|---|
| Sniper | 2.5 | MORS, Paladin HB50 |
| Marksman | 1.50 | MK14 |
| AR | 1.25 | AE4, Galil, AK-47 |
| LMG | 1.20 | M60, RPD |
| SMG | 1.10 | ASM1, PPSh-41, Chicom CQB, AK-74u |
| Pistol / Shotgun | 1.0 (untouched) | Five-Seven, RW1 / TAC-19, Olympia |

`weaponClass` buckets the GDT uses lump AR + MK14 + snipers under `"rifle"`, so those were split by hand. **Verified** every gun's live/pristine `hipSpreadMax` ratio matches its bucket (e.g. AK-47 7→8.75, M60 12→14.4, MK14 5→7.5; pistols/shotguns/snipers unchanged). Supersedes the earlier sniper-only tool (it reuses that tool's pristine backups, so snipers re-derive to ×2.5). `gdtdb /update` + `-GscOnly` build.

### Fixed — Base RPD scope drifted off-center under fire: inverted ADS pitch (user 2026-06-29)

The base RPD's sight drifted **off-center while firing** — the only gun affected. Root cause: the BOCW RPD port ships a **malformed `adsGunKickPitch`** range — `Max = -35` **below** `Min = -10` (negative + inverted); `apply_recoil_overhaul` scaled it to `Max = -61.25` vs `Min = -17.5`, so the gun model pitched hard the **same direction every shot** → the scope tilted off-center. (`symmetrize_cw_recoil.js` had centered the *yaw* but deliberately leaves pitch alone — a normal upward kick is fine; only this inverted range is the bug.) New [tools/fix_inverted_gunkick_pitch.js](tools/fix_inverted_gunkick_pitch.js) sets `Max = -Min` wherever the ADS gun-kick pitch range is inverted → symmetric **±17.5** (matching the M60). Applied to the RPD's base + PaP forms, the `.acc-orig` **baseline** (so a recoil-overhaul re-run stays fixed), and its **14 twins** — **RPD-only, ADS-only** (hip ranges + other guns untouched; verified the broad hip-inversion matches were excluded). `gdtdb /update` + `-GscOnly` build.

### Fixed — Cold War gun twins were missing their magazine (user 2026-06-28)

The perk-handling/PaP **twins** of the 4 Cold War guns (`t9_ak47/ak74u/m60/rpd`) rendered **without a magazine** — visible holding the RPD, and reloading the M60 (a fast-reload twin swaps in for the reload anim, so the mag vanished mid-reload — read as a base-gun bug, but it was the twin). Root cause: the twins ([source_data/acc_weapon_variants.gdt](source_data/acc_weapon_variants.gdt), cloned by `gen_weapon_variant_gdt.js` which copies the base block verbatim) were generated **before** the CW magazine attachment (`attachViewModel5`/`attachWorldModel5` = `wpn_t9_<gun>_mag_view/world`) was wired into the base GDTs, so all **56** twins (4 guns × 14) carried **empty** mag slots. The mag xmodels were already packed — only the twin fields were blank. New [tools/fix_twin_attachments.js](tools/fix_twin_attachments.js) `[--audit]` re-syncs every twin's attachment slots to its current installed base (stats untouched); **112 fields** (56 view + 56 world) restored. Deploy: `gdtdb /update` → `-GscOnly` build. **Audio verified clean** — comprehensive t9 fire+reload aliases, 94 wavs installed, valid GDT `fireSound` refs. The CW guns' one asset gap — the missing `t9_shell_eject_*.efx` (brass-casing FX) — was also fixed: [tools/fix_cw_shell_eject_fx.js](tools/fix_cw_shell_eject_fx.js) repoints all 4 eject FX fields on each gun's **base + PaP forms AND its 14 twins** (**256 refs**) to caliber-matched **stock MWR brass ejects** that ship on disk (`_scobalula\shellejects\mwr\`: AK47→`762x39`, AK74u→`545x39`, M60→`762x51`, RPD→the dedicated `762x39_rpd`). Install-side GDT + repo-twins edit (re-runnable) → `gdtdb /update` → build. **Gun SFX/model QA (full-build errorlog, user 2026-06-29):** clean of all gun FX/model/sound breaks — the twins were caught there (a base-only fix had left them on the dead FX, same clone-lag as the magazine). The 3 remaining errorlog lines are benign + non-gun: the long-waived `mtl_origins_camo_alt` (Five-Seven PaP camo material — substituted), a harmless Action-Figure *melee* impactsfxtable warning, and stock zombie/PaP/gib low-poly-mesh perf notes.

### Fixed — Shield (Riot) elite spawned at sprint and never slowed (user 2026-06-28)

The Shielded "Riot" elite ([_acc_elites.gsc](scripts/zm/zm_abandoned_cyber_city/_acc_elites.gsc) `shielded_speed_think`) is meant to be a heavy half-pace **walker**, but it spawned faster than the horde and never changed — in non-god mode it clobbered the player. Root cause: a spawn-order race against a stock **override-lock**. The frame-N+1 on-spawn hook (`_acc_zombie_speed::on_zombie_spawned_speed`) writes the horde tier (`"sprint"` at round ≥15) into `self.zombie_move_speed_override`; stock `set_zombie_run_cycle()` then early-returns whenever that override is already defined, so the think's `set_zombie_run_cycle_override_value("walk")` was **silently dropped** and the elite kept the sprint gait at ≥1.0 anim rate forever. Fix: the think now **clears the override-lock then re-asserts "walk"** (mirroring `_acc_zombie_speed::apply_speed_for_round`'s clear-then-set, guarded on drift), and the keep-alive skip-flag `acc_boss_custom_speed` is set **synchronously** in `promote_to_shielded` so the spawn hook never tags the actor. `-GscOnly` build.

### Changed — Plaza spawn crates are now Data Caches: 1 shard/round each (user 2026-06-28)

The 3 decorative cargo-crate cover obstacles in the spawn plaza ([zm_abandoned_cyber_city.gsc](scripts/zm/zm_abandoned_cyber_city.gsc) `acc_spawn_plaza_props`) now grant **Data Shards** — the user wanted the same crates the trench uses to actually be lootable. Each is spawned via `acc_data_shards::spawn_cache_at(origin, 1)` instead of a bare prop, so it's a hold-USE **Data Cache**: **1 shard** on loot, depletes, **re-arms at each round start** (with the dim-white "armed" glow). Same model + same origins → the `.map` collision clips (gen_plaza_shrink) and the silhouette are unchanged; the crates are still low cover, just functional now. Adds an **early-game shard faucet at spawn** (previously shards came only from the 2 underground caches + elite kills). A poll on `level.acc_shards_cache_model` makes it init-order-independent. GSC-only build. Co-op note: the shared `acc_cache_one_per_player` anti-hog cap spans these + the 2 trench caches (SOLO is exempt → all 3 grabbable/round).

### Fixed — Corp-trench bridge rendered transparent → script_brushmodel (user 2026-06-28)

The 2×-jump **corp-trench bridge** (slab @ z+58 over the Bus Station trench) drew **transparent** in-game ("transparent like it doesn't have a texture"). Root cause: a thin **floating world slab** gets a broken/empty LED lightmap, so the surface composites **see-through**. The geometry was never deleted — it was a **bake** artifact. Fix (memory `brushmodel-wall-led-exempt`): [tools/convert_bridge_to_brushmodel.js](tools/convert_bridge_to_brushmodel.js) moves the brush out of worldspawn into a **script_brushmodel** `acc_corp_bridge` — brushmodels are **LED-exempt**, so it renders with runtime/dynamic lighting and skips the broken bake. [_acc_bus_trench.gsc](scripts/zm/zm_abandoned_cyber_city/_acc_bus_trench.gsc) `solidify_corp_bridge()` show+solids it at load (script_brushmodels start hidden+notsolid, `_acc_decontamination.gsc:39`). Also re-skinned to a readable **diamond-plate** walkway (was the dark Corp marble, which blended into the trench). FULL LED build (BSP+navmesh regenerate; bake passed). Players cross by jumping up; zombies never pathed the elevated slab, so dropping it from the world navmesh is harmless.

### Added — Atmosphere & feel pass: neon lighting, reflections, FX, hero-glows, power-on (user 2026-06-28)

A 5-phase atmosphere pass over the per-zone textures, each **independently revertable**:
- **Phase 1 — neon lighting** ([tools/gen_neon_lights.js](tools/gen_neon_lights.js)): **22** colored, **power-gated** accent-light pools keyed per zone (cyan Plaza/Corp, magenta Market, red Alley, amber Helipad, cyan+amber Vault, cyan+magenta Lab, blue Trench) — saturated pools with darkness between (noir). Dark pre-power → **ignite on power**. Baked (LED). Revert: `--revert`.
- **Phase 2 — reflection probes** ([tools/gen_reflection_probes.js](tools/gen_reflection_probes.js)): **+8** (15 total) so the wet floors mirror the neon. Baked by the LED `+recompute` pass (KB-confirmed). Revert: `--revert`.
- **Phase 3 ambient FX + Phase 4 hero neon-glows** ([_acc_atmosphere.gsc](scripts/zm/zm_abandoned_cyber_city/_acc_atmosphere.gsc) `apply_fx`): drifting interior dust-haze + factory steam-leak vents (Alley/Vault/Trench); our own looping `fx_perk_glow_*` reused as the visible neon **sources** at signage spots (corp logo, market ads, vault racks, lab machines). Server `PlayFX`, gated by `acc_atmo_fx`.
- **Phase 5 — polish:** the power-on transformation is **emergent** — the existing fog **settles away**, the light **warm-up ramp**, and the new neon **igniting** all fire when power flips; the tuned cold-haze fog is unchanged.
- Builds: a FULL LED build (lights + reflections baked) then a GSC-only build (FX). Both clean — all FX pack, bake passed.
- **NOTE:** FX use *server* `PlayFX`; our notes flag server FX as sometimes flaky for persistent effects — if glows/steam don't render in-game they promote to a client `_acc_atmosphere_fx.csc` (lights + reflections are **baked**, so guaranteed).
- **Revert-all:** `gen_neon_lights.js --revert` + `gen_reflection_probes.js --revert` + `set acc_atmo_fx 0` + rebuild → back to the textured-only map.

### Added — Per-zone material art pass: every zone gets its own texture identity (user 2026-06-28)

Replaced the uniform near-black asphalt with a **per-zone material scheme** matching each zone's documented art direction (docs/29 §5), so the map reads as distinct, named spaces. `tools/paint_region.js` was rebuilt to apply **per-surface materials** — walls vs floor vs ceiling, via the vertical/flat orientation test — per region; zone bounds were scoped with the new **`tools/list_zones.js`** (player_volume footprints) + `find_region.js`. Regions are ordered **peripherals-first** so the central Corp hub yields the shared border walls to its neighbours. **187 brushes / 1122 faces** repainted; all **11** stock materials (harvested from Treyarch prefabs, 806 map sources / 222 distinct) **ship** — no missing-material warning; FULL LED-baked build (passed). Floor-named textures stay on **floors only** (no repeat of the earlier cable-panel-on-walls mistake, now reverted/superseded here). Scheme:
- **Plaza** grimed `concrete_wall_weathered_01_wet` walls + cracked `concrete_floor_garage_cracked_wet_nw`
- **Corp / Bus Station** (hub) sleek `metal_bare_graphite_matte` + `stone_marble_dark_01` floor
- **Market** `metal_paint_rust_brown` + `concrete_tiles_2x2_dirty_01_wet` floor
- **Alley** `metal_painted_wall_dirty_black` (floor kept wet asphalt)
- **Vault** `metal_panel_2x1_stainless_steel_brushed` + `metal_diamond_plate_worn_wet` grating
- **Helipad** `concrete_wall_poured_thick_01_wet` + `concrete_wall_weathered_01_wet` floor
- **Trench** `metal_worn_iron_dark` (industrial under-level)
- Kept: **Lab / Paradise** hex tile; **Abyss** dark asphalt (intentional).
- **Trench gap fix** (user spotted patchy textures in the bus-station trench): the Trench spans a taller z-range than one region covered, so 7 brushes (2 stair pit-walls `z[-240,128]`, 2 under-room floor-lids `z[-96,0]`, 3 doors) fell in a z-gap between Trench (≤ −90) and Corp (≥ −30) and kept the asphalt. Closed it (Trench `z2` −90 → **−25**, still below the Corp floor) + ran **`--entities`** — which also (intentionally) gives every zone's **buyable doors** their zone material. Re-verified **0** asphalt left in the trench column. **NEXT:** glass "hero" walls (Corp logo, Vault rack-glow) as a targeted pass; optional Lab walls → clean-white.

### Added — Lab + Paradise reskinned clean hex-tile white (bounds-aware, user 2026-06-28)

The **Lab** room and the **Paradise** hallway + open-air plaza are repainted from the global near-black asphalt to the clean light **`t7_zm_der_tile_hexagon`** (Der Eisendrache hex tile — "lab/corp tech" read, a "lab coat white" look), while the rest of the map stays dark. Since the global paint left every face on one token, this is a **bounds-aware** swap via the new **[tools/paint_region.js](tools/paint_region.js)**: it repaints only the worldspawn brushes whose centroid lands in a region AABB. Regions (scoped with the new read-only inspector **[tools/find_region.js](tools/find_region.js)**): **Lab** `x[-790,830] y[3040,4260] z[-30,300]` (17 brushes — floor/walls/ceiling/perk-alcoves), **Paradise hallway** `x[-120,120] y[-590,1700] z[-1220,-980]` (4), **Paradise plaza** `x[-1030,1030] y[-2230,-575] z[-1220,-180]` (7) = **28 brushes / 168 faces**. The hall/plaza boxes are deliberately **z-tight** so they catch the deep hub and NOT the trench/abyss/basement that share x,y above them. `t7_zm_der_tile_hexagon` was "confirm-in-APE" — this build **confirms it ships** (linker threw no missing-material warning). FULL LED-baked build (bake passed). The 11 Lab perk-**blocking door** brushes (`_acc_perk_doors`, entity/script_brushmodel) were then folded in too: `paint_region.js` gained a `--entities` opt-in to repaint in-region brushmodels (user 2026-06-28), so the Lab is now fully hex (shell + doors). Still dark (extend if wanted): the Lab approach connectors (`c_roof_lab`, `c_vlt_lab`).

### Added — Whole map reskinned: walls + floors + ceilings painted near-black (user 2026-06-28)

Every greybox FACE is reskinned from its placeholder token (which rendered as the checker "missing material" pattern) to the stock near-black wet asphalt **`t7_asphalt_damaged_dark_wet`** (docs/29 palette `#0E1115`), via the new **[tools/paint_walls.js](tools/paint_walls.js)** — a whole-word token swap of `script_wall` (**1152** WALL faces) + `script_floor_ceiling` (**1092** FLOOR + CEILING faces) = **2244** faces, so **no greybox checker remains anywhere** (the only non-asphalt face tokens left are the invisible tool textures `clip`/`trigger`/`volume`/`sky`). Verified first that both tokens only ever appear as brush-face materials (never entity keys/values), so the swaps can't touch anything else. A face's material token IS the GDT material name and stock `t7_*` materials ship free with **no `.zone` line** (docs/29 / CLAUDE.md), so this is a pure `.map` swap — but BSP-baked, so a FULL cod2map + **LED bake (passed — material swaps don't change brush winding, so no `brush.cpp:1860` risk)** + linker. Linker reported **no** missing-material warning for the asphalt token, confirming it ships. `paint_walls.js` now takes `[FROM] [TO]` args so any surface repaints (e.g. `paint_walls.js script_floor_ceiling <mat>`); revert via `git checkout` the `.map`. **NEXT:** the Lab zone gets a clean-white reskin — a bounds-aware pass, since all faces now share one token. Completes the docs/29 Phase-1 wall+floor skin.
