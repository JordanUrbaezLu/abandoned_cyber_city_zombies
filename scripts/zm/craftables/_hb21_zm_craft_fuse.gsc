...
function fuse_set_glow()
{
	...
	foreach ( str_fuse in a_fuses )
	{
		...
		// [acc] GUARD (2026-07-02): our map places NO fuse prefabs, so the piece model is
		// undefined -> this line threw "undefined is not an object" x3 every game (console
		// spam; the glow is fuse-quest-only cosmetics). Pack bug, isdefined-guarded.
		if ( isdefined( e_fuse ) )
