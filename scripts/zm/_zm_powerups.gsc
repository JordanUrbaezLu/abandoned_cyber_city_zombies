// [acc] STOCK-SCRIPT OVERRIDE: eMoX "T8 Powerup Delayed Drop" (installed 2026-07-11).
// This is the stock _zm_powerups.gsc + eMoX's BO4-style pre-drop: powerup_setup() plays a 1.5s
// "incoming" FX + loop sound (macros in _zm_powerups.gsh) BEFORE the model appears / grab arms.
// Same-path replacement: our zone's scriptparsetree wins over zm_patch at load; the matching
// stock line in <tools>\zone_source\all\assetlist\zm_patch.csv is commented out so the linker's
// dedupe doesn't silently drop this copy (install-side edit - see external_assets_manifest.ps1).
// BEHAVIORAL NOTE for callers: specific_powerup_drop()/powerup_drop() now BLOCK ~1.6s inside
// powerup_setup - callers must be `level thread`ed (an entity-owned thread dying mid-delay
// strands a model-less powerup that never times out). All repo call sites audited 2026-07-11.
// [acc] edits vs the pack: only_affects_grabber IS_TRUE hardening (undefined-compare trap).
...
function powerup_setup( powerup_override,powerup_team, powerup_location, powerup_player, shouldplaysound = true, is_monk_cycling = false )
{
	...
	// eMoX EDIT : 18/04/2020 - Code updated 14/12/2025
	if(isdefined(POWERUP_TIMED_DROP) && IS_TRUE(POWERUP_TIMED_DROP))
	{
	 	if(!is_monk_cycling)
	 	{
			if( IS_TRUE( struct.only_affects_grabber ) ) // [acc] was `== true` - throws/misbehaves when the registration left it undefined
			{
