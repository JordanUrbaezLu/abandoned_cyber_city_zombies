...
init()
{
    _acc_utility::log( "events_overload init" );
...
watch_terminal()
{
    ...
    if ( triggers.size == 0 )
    {
        _acc_utility::log( "overload: no terminal placed yet" );
        return;
    }
...
terminal_loop()
{
    ...
    {
        _acc_utility::log( "overload: no point struct placed" );
        return;
    }
    ...
    for ( ;; )
    {
        ...
        player.score -= ACC_OVERLOAD_ACTIVATION_COST_POINTS;
        ...
        if ( result == "success" )
        {
            _acc_data_shards::grant_player( player, ACC_OVERLOAD_REWARD_SHARDS, "overload" );
            unlock_map_shortcut();
...
run_overload( player, point_struct )
{
...
is_player_on_point( player, point_struct )
{
...
overload_wave_schedule( at_sec, wave_level )
{
    wait( at_sec );

    // Each wave adds ~4 zombies + an elite forced on waves 2/3.
    zombie_count = 4 + ( 2 * wave_level );
    _acc_utility::log( "overload: wave " + wave_level + " (" + zombie_count + " zombies)" );
...
spawn_takedown_wave()
{
    _acc_utility::log( "overload: takedown wave (3 of each elite class)" );
...
unlock_map_shortcut()
{
    _acc_utility::log( "overload: shortcut Server->Roof unlocked" );
