...
function bow_base_wield_watcher( str_bow )
{
	...
	while ( 1 )
	{
		...
		// [acc] TWIN-COMPAT (2026-07-08): compare by bow FAMILY name, not object identity - a held
		// perk-twin form ("elemental_bow_demongate_acc_fastreload", Mega Speed Cola) is a different
		// weapon OBJECT but the same bow; the object test made the ambient-FX/hint wield path dead
		// for twin holders. bow_base_get_bow_weapon strips twin + charge-level suffixes.
		if ( ( wpn_new === w_bow )
			 || ( isdefined( wpn_new ) && isdefined( wpn_new.name ) && bow_base_get_bow_weapon( wpn_new.name ) == str_bow ) )
		{
			...
			self clientfield::set_to_player( str_bow + "_ambient_bow_fx", 1 );
			 ...
			 || ( isdefined( wpn_old ) && isdefined( wpn_old.name ) && bow_base_get_bow_weapon( wpn_old.name ) == str_bow ) )
		{
...
function bow_base_get_bow_weapon( str_weapon_name )
{
	...
	// [acc] TWIN-COMPAT (2026-07-08): strip our perk-twin suffix FIRST so a Mega-Speed-Cola bow form
	// ("elemental_bow_demongate_acc_fastreload") resolves to its bow family - without this, a twin
	// holder's TAP shots failed the family match in bow_base_impact_watcher (no chomper). Charged
	// shots were unaffected (the projectile weapon is the charge-level def, never the twin).
	if ( isSubStr( w_player_bow, "_acc_" ) )
	{
		w_bow_parent = strtok2( w_player_bow, "_acc_" );
