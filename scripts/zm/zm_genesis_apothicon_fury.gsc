...
function apothicon_fury_spawn( v_origin, v_angles = ( 0, 0, 0 ), b_find_flesh = 0 )
{
	...
	{
		// [acc] NO endon("death") here: apothicon_fury_spawn() is called INLINE for its return value
		// (_acc_fury.gsc:254, :180), so an endon would bind to the CALLER's per-player spawn timer and
		// kill it when the first fury dies - no more furies for that player. The only wait is the
		// b_find_flesh branch below, now isdefined-guarded instead.
		...
		{
			wait 1;
			if ( isdefined( e_ai_apothicon_fury ) )   // [acc] guard: fury can die/be reaped during the wait
