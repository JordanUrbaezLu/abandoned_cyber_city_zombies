...
function __init__()
{
	...
	clientfield::register( "scriptmover", "robot_switch", VERSION_SHIP, 1, "int" );
	clientfield::register( "world", "robot_lights", VERSION_SHIP, 2, "int" );
	// # CLIENTFIELD REGISTRATION
	...
	zm::register_player_damage_callback( &zod_robot_player_damage_override );
	// # REGISTER CALLBACKS / OVERRIDES
...
function zod_robot_player_damage_override( eInflictor, eAttacker, iDamage, iDFlags, sMeansOfDeath, weapon, vPoint, vDir, sHitLoc, psOffsetTime )
{
	// [acc] FIXED 2026-07-02 (GOD-MODE KILLER): stock chain semantics - _zm.gsc
	// check_player_damage_callbacks returns the FIRST callback result != -1 and SKIPS the rest.
	// The pack's original `return iDamage` here short-circuited EVERY later player-damage
	// callback, including _acc_elites' god-mode zeroing ("I was dying in god mode"). Return -1
	// = "no opinion", let the chain continue. (The pack's ally no-damage guard checked the
	// TYPO'd field `.archeype` so it never fired anyway - and it must STAY dead for the
	// ROGUE PROTECTOR boss, whose archetype IS zod_companion and whose bullets must damage
	// players; his hit strength is tuned in the weapon GDT playerDamage field instead.)
	return -1;
}
...
{
	if ( IS_TRUE( level.reviving_a_player ) )
		return 0;
...
function zod_robot_callbox_wait_for_crafted()
{
	...
	// [acc] #9 GUARD (2026-07-05): the clientfield::set was OUTSIDE the isDefined check, so a missing
	// police_box entity (the fuse EE places one; the map may not) threw "undefined is not an entity" -
	// the exact crash the sibling _hb21_zm_craft_fuse guards against. Fold both into one guarded block.
...
function zod_robot_callbox_visibility_func( e_player )
{
	...
	if ( !level flag::get( "police_box_ready" ) )
		self setHintString( "Civil Protector Offline" );
		...
		self setHintString( "Hold ^3[{+activate}]^7 to Contribute to the Civil Protection Fund." );
	else
		self setHintString( "Hold ^3[{+activate}]^7 to Summon the Civil Protector." );
...
function zod_robot_callbox_logic_func()
{
	while ( 1 )
	{
		...
		else
		{
			...
			wait 1.5;
			// [acc] 4p guard (2026-07-06 sweep): purchaser can disconnect during the 1.5s; the
			// unitrigger thread only endons "kill_trigger", so the deref threw.
...
function zod_robot_summon( e_player, e_trig_stub, n_spawn_delay )
{
	...
	if ( !isdefined( level.ai_robot ) )   // [acc] COOP CRASH GUARD: spawnFromSpawner returns undefined when the
		return;                           // actor pool is full (4p horde pressure); the field writes below would throw
	...
	// [acc] (2026-07-06 sweep): loop var renamed - it SHADOWED the purchaser param, so after the loop
	// e_player was the LAST player in level.players, and line ~261 passes e_player onward. Harmless
	// today only because zod_robot_do_landing re-shadows its param; a loaded gun for any future edit.
	foreach ( p in level.players )
		p setPerk( "specialty_pistoldeath" );
	...
	foreach ( p in level.players )
	{
		p unsetPerk( "specialty_pistoldeath" );
	}
