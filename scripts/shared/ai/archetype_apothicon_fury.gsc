...
function mocompApothiconFuryBamfTerminate( entity, mocompAnim, mocompAnimBlendOutTime, mocompAnimFlag, mocompDuration )
{
	...
	entity Solid();
	entity Show();
	// [acc] disarm the stuck-ghost failsafe AND restore the juke clientfield: stock terminate
	// re-Shows the entity but never resets FURY_JUKE_CLIENTFIELD, so a bamf cut between the
	// out/in notetracks left the client-side "teleported out" state (looping FX off) stuck at 0.
	entity notify( "acc_fury_bamf_watchdog" );
	entity clientfield::set( FURY_JUKE_CLIENTFIELD, 1 );
	entity.blockingPain = false;
	entity.useGoalAnimWeight = false;
...
function apothiconBamfOut( entity )
{
	...
	// [acc] STUCK-GHOST FAILSAFE (user 2026-07-09: "fury uses its charge attack and then goes
	// invisible"). Ghost()/NotSolid() here are only undone by the FURY_BAMF_NT_STOP notetrack
	// (apothiconBamfIn) or the bamf MOCOMP's terminate - but the bamf notetracks also live on
	// TRAVERSAL anims (see apothiconBamfIn's traverseEndNode branch), which have NO mocomp
	// terminate safety net, and an anim interrupted/blended out between the two notetracks skips
	// the restore entirely -> a permanently invisible, non-solid fury still charging players.
	// Arm a watchdog every bamf-out; bamf-in / mocomp-terminate disarm it.
	entity notify( "acc_fury_bamf_watchdog" );          // restart: kill any prior watchdog
	entity thread acc_bamf_ghost_failsafe();
...
{	
	...
	entity notify( "acc_fury_bamf_watchdog" );   // [acc] bamf completed normally - disarm the stuck-ghost failsafe
...
// [acc] STUCK-GHOST FAILSAFE (user 2026-07-09) - self = a fury that just bamfed OUT (Ghost + NotSolid).
// Armed by apothiconBamfOut; disarmed (notify) by apothiconBamfIn or the bamf mocomp terminate. The
// longest legit bamf/traverse ghost window is well under 2s - if 4s pass with neither restore firing
// (the anim was interrupted between the out/in notetracks, e.g. a cut traversal), force the fury back:
// visible, solid, and the juke clientfield back to 1 so the client-side looping FX resume. Without this
// the fury stays permanently invisible AND unhittable (NotSolid) while still attacking.
function acc_bamf_ghost_failsafe()
{
	self endon( "death" );
	self endon( "acc_fury_bamf_watchdog" );
